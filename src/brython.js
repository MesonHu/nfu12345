 ...
 implementation [3, 10, 6, 'dev', 0]
 version compiled from commented, indented source files at
 github.combrython-devbrython
...
$B.url2name={}
$B.$py_src={}
$B.path=[$path+'Lib',$path+'libs',$script_dir,$path+'Libsite-packages']
...
$B.special_string_repr={8x08,9t,10n,11x0b,12x0c,13r,92,160xa0}
...
klass.$infos={__name__name,__module__module,__qualname__name}}
klass.__module__=module
for(var attr in klass){if(typeof klass[attr]=='function'){klass[attr].$infos={__doc__klass[attr].__doc__ ,__module__module,__qualname__ name+'.'+attr,__name__attr}
...
var $=$B.args('__repr__',1,{selfnull},['self'],args,{},null,null),self=$.self,_b_=$B.builtins
...
js=(function() {n var $locals_+script_id+ = {}n+js+n}())
return js}
...
__BRYTHON__.compiled_date=2022-05-09 111554.908810
...
return res}
var errors={}
function TokenError(message,position){if(errors.TokenError===undefined){var $error_2={$nameTokenError,$qualnameTokenError,$is_classtrue,__module__tokenize}
...
__class__error,msgmessage,linenoposition[0],colnoposition[1]}}
...
var end=src.substr(pos).search([rn])
...
var t=[]
while(true){if(pos =src.length (ix='rn'.indexOf(src[pos]))-1){t.push(Token('COMMENT',src.substring(start-1,pos),[line_num,start-line_start],[line_num,pos-line_start+1],line))
if(ix !==undefined){var nb=1
if(src[pos]=='r' && src[pos+1]=='n'){nb++}else if(src[pos]===undefined){
nb=0}
t.push(Token(token_name,src.substr(pos,nb),[line_num,pos-line_start+1],[line_num,pos-line_start+nb+1],line))
if(src[pos]===undefined){t.push(Token('NEWLINE','n',[line_num,pos-line_start+1],[line_num,pos-line_start+2],''))}
pos+=nb}
return{t,pos}}
pos++}}
...
$B.tokenizer=function(src){var unicode_tables=$B.unicode_tables,whitespace=' tn',operators='+-%&^~=',allowed_after_identifier=',.()[];',string_prefix=^(ruRUfFfrFrfRFRrfrFRfRF)$,bytes_prefix=^(bBbrBrbRBRrbrBRbRB)$
...
line_start=pos
line_num++
if(mo=^f(rnrn).exec(src.substr(pos-1))){
...
continue}
indent=0
if(char==' '){indent=1}else if(char=='t'){indent=8}
if(indent){while(pos  src.length){if(src[pos]==' '){indent++}else if(src[pos]=='t'){indent+=8}else{break}
pos++}
if(pos==src.length){
line_num--
break}
...
for(var item of comment.t){yield item}
pos=comment.pos
continue}else if(mo=^f(rnrn).exec(src.substr(pos))){
...
break
case ''
if(mo=^f(rnrn).exec(src.substr(pos))){line_num++
pos+=mo[0].length
line_start=pos+1
...
case 'r'
var token_name=braces.length  0  'NL''NEWLINE'
mo=^f(rnrn).exec(src.substr(pos-1))
...
pos++}else if(char==' ' char=='t'){}else{yield Token('ERRORTOKEN',char,[line_num,pos-line_start],[line_num,pos-line_start+1],line)}}
break
case 'NAME'
...
line_num++
line_start=pos+1
if(char=='r' && src[pos]=='n'){string+=src[pos]
line_start++
pos++}
line=get_line_at(src,pos)
escaped=false
...
if(! src.endsWith('n')&& char !=' ' && state !=line_start){yield Token('NEWLINE','',[line_num,pos-line_start+1],[line_num,pos-line_start+2],'')
line_num++}
while(indents.length  0){indents.pop()
...
return res}
$B.op2method={operations{pow,floordiv,lshift,rshift,+add,-sub,mul,truediv,%mod,@matmul },augmented_assigns{=ifloordiv,=irshift,=ilshift,=ipow,+=iadd,-=isub,=imul,=itruediv,%=imod,&=iand,=ior,^=ixor,@=imatmul},binary{&and,or,~invert,^xor},comparisons{lt,gt,=le,=ge,==eq,!=ne},boolean{oror,andand,inin,notnot,isis},subsetfunction(){var res={},keys=[]
...
if(attrs.length  0){res+='n'
res+=attrs.map(x= '  '.repeat(indent+1)+x).join(',n')}
res+=')'
return res}
...
if(src){lines=src.split('n'),text=lines[position.start[0]-1]}
...
if(noassign[name]===true){report(keyword)}}else if(['str','int','float','complex'].indexOf(assigned.type)-1){report('literal')}else if(assigned.type==ellipsis){report('Ellipsis')}else if(assigned.type=='genexpr'){report('generator expression')}else if(assigned.type=='packed'){check_assignment(assigned.tree[0],{action,oncetrue})}else if(assigned.type=='named_expr'){report('named expression')}else if(assigned.type=='list_or_tuple'){for(var item of ctx.tree){check_assignment(item,{action,oncetrue})}}}else if(ctx.type=='list_or_tuple'){for(var item of ctx.tree){check_assignment(item,{action,oncetrue})}}else if(ctx.type=='ternary'){report('conditional expression')}else if(ctx.type=='op'){report('operator')}else if(ctx.type=='yield'){report('yield expression')}else if(ctx.comprehension){break}
if(once){break}
ctx=ctx.parent}}
...
var indentation='  ',lines=js.split('n'),level=indent,res='',last_is_closing_brace=false,last_is_backslash=false,last_is_var_and_comma=false
...
console.log(res)}
level=0}
try{res+=(add_spaces  indentation.repeat(level)'')+line+'n'}catch(err){console.log(res)
throw err}
if(line.endsWith('{')){level++}else if(add_closing_brace){level--
if(level  0){level=0}
try{res+=indentation.repeat(level)+'}n'}catch(err){console.log(res)
throw err}}
...
console.log('this',ctx,'nline',lnum,src.split('n')[lnum-1])}
...
check_assignment(C,{augmentedtrue})
...
var Comprehension={generatorsfunction(comp){
var comprehensions=[]
...
return comprehensions},make_compfunction(comp,C){comp.comprehension=true
...
Comprehension.set_parent_block(C.tree[0],comp)},set_parent_blockfunction(ctx,parent_block){if(ctx.tree){for(var item of ctx.tree){if(item.comprehension){item.parent_block=parent_block}
...
[assign,return].indexOf(C.parent.type)-1){if($parent_match(C,{typeyield,fromtrue})){$_SyntaxError(C,no implicit tuple for yield from)}
C.parent.tree.pop()
var tuple=new $ListOrTupleCtx(C.parent,'tuple')
tuple.implicit=true
tuple.has_comma=true
...
try{check_assignment(item,{oncetrue})}catch(err){$_SyntaxError(C,[invalid syntax. +
...
var res={modulemodule undefined,names[],level}
...
while(line_start-- 0 && source[line_start]!='n'){}
err.args[1][2]+=value.start+save_pos-line_start
err.lineno+=line_num-1
err.args[1][3]=$get_module(this).src.split('n')[line_num-1]
throw err}
$pos=save_pos
...
state='string'}else{var conv_num={a97,r114,s115},format=item.elt.format
...
this.position=$token.value
C.tree[C.tree.length]=this}
$NumberCtx.prototype.ast=function(){var ast_obj=new ast.Constant({typethis.type,valuethis.value})
set_position(ast_obj,this.position)
return ast_obj}
...
case '+'
C.expect=','
return new $PatternLiteralCtx(C,{signvalue})
case ''
C.expect='starred_id'
return C
default
...
C.parent.tree.pop()
C.parent.tree.push(this)
if(token.sign){this.tree=[{signtoken.sign}]
this.expect='number'}else{if(token=='str'){this.tree=[]
new $StringCtx(this,value)}else if(token=='JoinedStr'){$_SyntaxError(this,[patterns cannot include f-strings])}else{this.tree=[{typetoken,value,sign}]}
this.expect='op'}}
...
C.tree.push({typetoken,value,signC.num_sign})
return C.parent
default
...
this.parent=C
this.position=$token.value
function prepare(value){value=value.replace(ng,'nn')
value=value.replace(rg,'rr')
return value}
...
var node=this.parent.node,res={bodyast_body(this.parent),handlers[],orelse[],finalbody[]}
...
if($parent_match(C,{typeannotation})){$_SyntaxError(C,['yield' outside function])}
var parent=this
while(true){var list_or_tuple=$parent_match(parent,{typelist_or_tuple})
...
var parent=this
while(true){var set_or_dict=$parent_match(parent,{typedict_or_set})
...
node.has_yield=this
var in_comp=$parent_match(this,{typecomprehension})
...
var s_escaped='abfnrtvxuU0123456789'+'+'',is_escaped={}
...
function test_num(num_lit){var len=num_lit.length,pos=0,char,elt=null,subtypes={b'binary',o'octal',x'hexadecimal'},digits_re=[_d]
...
elt.value=elt.value.replace(_g,)
elt.length=pos
return elt}}
while(pos  len){var char=num_lit[pos]
if(char.match(digits_re)){if(elt===null){elt={valuechar}}else{if(char=='_' && elt.value.match([._+-]$)){
...
if(elt.subtype==x){digits_re=[_da-fA-F]}
elt.value=''
...
elt.value=elt.value.replace(_g,)+char
pos++}else{return check(elt)}}else if(char.match(ei)){if(num_lit[pos+1]===undefined){error(nothing after e)}else if(elt && subtypes[elt.subtype]!==undefined){
...
elt.value+=char
elt.exp=true
pos++}else{error(unexpected e)}}else{return check(elt)}}else if(char.match([+-]i)){if(elt===null){elt={valuechar}
pos++}else if(elt.value.search(e$i)-1){elt.value+=char
pos++}else{return check(elt)}}else if(char.match(ji)){if(elt &&(! elt.subtype elt.subtype==float)){elt.imaginary=true
check(elt)
elt.length++
...
n=n.replace(_g,)
if(n.startsWith('.')){if(n.endsWith(j)){return{type'imaginary',valueprepare_number(n.substr(0,n.length-1))}}else{return{type'float',valuen}}
pos=j}else if(n.startsWith('0')&& n !='0'){
var num=test_num(n),base
if(num.imaginary){return{type'imaginary',valueprepare_number(num.value)}}
if(num.subtype=='float'){return{typenum.subtype,valuenum.value}}
...
type'imaginary',valueprepare_number(num.value)}}else{return{
type'float',valuenum.value}}}else{if(num.imaginary){return{
type'imaginary',valueprepare_number(num.value)}}else{return{
...
while(end  src.length){if(escaped){if(src.charAt(end)==a && ! raw){zone=zone.substr(0,zone.length-1)+u0007}else{zone+=src.charAt(end)
...
escaped=true}else{if(src.charAt(end+1)=='n'){
...
zone+=''
escaped=true
end++}}}}else if(src.charAt(end)=='n' && _type !='triple_string'){
...
var lines=src.split('n'),line,global_indent,indent,unindented_lines=[]
...
line.match(s).length+'n    '+line)}}else{unindented_lines.push('')}}
return unindented_lines.join('n')}
...
var line2pos={00,10},line_num=1
for(var pos=0,len=src.length;pos  len;pos++){if(src[pos]=='n'){line_num++
...
js_from_ast=' ast generated by parsern'+js_obj.js
return{
importsjs_obj.imports,to_jsfunction(){return js_from_ast}}}else{var root=$create_root_node(
{srcsrc,has_annotationshas_annotations,filenamefilename},module,locals_id,parent_scope,line_num)
...
if(options===undefined){options={debug1}}else if(typeof options=='number'){options={debugoptions}}else if(typeof options !=='object'){console.warn('ignoring invalid argument passed to brython()',options)
options={debug1}}
...
observer.observe(document.documentElement,{childListtrue,subtreetrue});}
function checkPythonScripts(addedNode){if(addedNode.tagName=='SCRIPT' && addedNode.type==textpython){var options={}
...
if(script.type==textpython script.type==textpython3){if(script.className==webworker){if(script.id===undefined){throw _b_.AttributeError.$factory(
...
var $trace=$B.$getattr($err,'info')+'n'+$err.__name__+
' '+$err.args
...
if(worker.src){
$B.tasks.push([$B.ajax_load_script,{nameworker.id,urlworker.src,is_wwtrue}])}else{
src=(worker.innerHTML worker.textContent)
src=unindent(src)
src=src.replace(^n,'')
$B.webworkers[worker.id]=src}}
for(var i=0;i  $elts.length;i++){var elt=$elts[i]
if(elt.type==textpython elt.type==textpython3){
...
if(elt.src){
$B.tasks.push([$B.ajax_load_script,{namemodule_name,urlelt.src,idelt.id}])}else{
...
$B.url2name[url]=name
try{var root=$B.py2js({srcsrc,filenameurl},name,name),js=root.to_js(),script={__doc__root.__doc__,jsjs,__name__name,$srcsrc,__file__url}
...
if(run_loop){$B.loop()}}
var $log=$B.$log=function(js){js.split(n).forEach(function(line,i){console.log(i+1,,line)})}
...
{srcsource,filenamemodule},module,module),js=root.to_js()}catch(err){$B.handle_error(err)}
...
var db=$B.idb_cx.result,tx=db.transaction(modules,readwrite),store=tx.objectStore(modules),cursor=store.openCursor(),data={namemodule,contentjs,importsimports,originorigin,timestamp__BRYTHON__.timestamp,source_tssource_ts,is_packageis_package},request=store.put(data)
...
var db=$B.idb_cx.result,store=db.createObjectStore(modules,{keyPathname})
...
var db=idb_cx.result,store=db.createObjectStore(modules,{keyPathname})
...
' from cache'}))}})(module)}
document.dispatchEvent(new CustomEvent('precompile',{detailclose}))
...
var module=new Function(script.js+`nreturn locals`)()
...
console.log(script.js.split('n').
slice(lineNumber-4,lineNumber).join('n'))}
...
var object={
$infos{__name__object},$is_classtrue,$nativetrue}
...
object.__format__=function(){var $=$B.args(__format__,2,{selfnull,specnull},[self,spec],arguments,{},null,null)
...
clmethod.$infos={__self__cls,__func__res,__name__res.$infos.__name__,__qualname__cls.$infos.__name__+.+
res.$infos.__name__}
return clmethod}
...
method.$infos={__self__self,__func__res,__name__attr,__qualname__klass.$infos.__name__+.+attr}
...
_reconstructor.$infos={__qualname___reconstructor}
...
__newobj__.$infos={__name____newobj__,__qualname____newobj__}
...
can't set attributes of built-inextension type 'object')}else if(self.__class__===object){
...
object.$factory=function(){var res={__class__object},args=[res].concat(Array.prototype.slice.call(arguments))
object.__init__.apply(null,args)
return res}
$B.set_func_names(object,builtins)
$B.make_class=function(qualname,factory){
var A={__class___b_.type,__mro__[object],$infos{__qualname__qualname,__name__$B.last(qualname.split('.'))},$is_classtrue}
A.$factory=factory
return A}
...
var class_dict={__bases__bases,__class__metaclass,__dict__cl_dict}
...
var kls=meta_new(metaclass,class_name,bases,cl_dict,{$nat'kw',kwextra_kwargs})
kls.__module__=module
kls.$infos={__module__module,__name__class_name,__qualname__class_obj.$qualname}
kls.$subclasses=[]
...
meta_method.$infos={__self__klass,__func__res,__name__attr,__qualname__klass.$infos.__name__+.+attr,__module__res.$infos  res.$infos.__module__ }
...
result.$infos={__func__res,__name__res.$infos.__name__,__qualname__klass.$infos.__name__+.+res.$infos.__name__,__self__klass}}else{result=res.__get__(klass)}
...
type.__name__={__get__function(self){return self.$infos.__name__},__set__function(self,value){self.$infos.__name__=value},__str__function(self){return type},__eq__function(self,other){return self.$infos.__name__==other}}
...
var class_dict={__class__ meta,__bases__ bases,__dict__ cl_dict,$infos{__name__name,__module__module,__qualname__name},$is_classtrue,$has_setattrcl_dict.$has_setattr}
...
return class_dict}
type.__or__=function(){var $=$B.args('__or__',2,{clsnull,othernull},['cls','other'],arguments,{},null,null),cls=$.cls,other=$.other
...
type.__qualname__={__get__function(self){return self.$infos.__qualname__ self.$infos.__name__},__set__function(self,value){self.$infos.__qualname__=value},__str__function(self){console.log(type.__qualname__)},__eq__function(self,other){return self.$infos.__qualname__==other}}
...
apply(null,arguments)}}}else if(klass.hasOwnProperty(__init__)){factory=function(){var obj={__class__klass,__dict__$B.empty_dict()}
klass.__init__.bind(null,obj).apply(null,arguments)
...
var res=Object.create(null)
$B.update_obj(res,{__class__klass,__dict__$B.empty_dict()})
...
factory.$infos={__name__klass.$infos.__name__,__module__klass.$infos.__module__}
return factory}
...
f.$infos={__name__attr,__module__klass.__module__}
return f}
)
method_wrapper.__str__=method_wrapper.__repr__=function(self){return method '+self.$infos.__name__+' of function object}
var member_descriptor=$B.make_class(member_descriptor,function(attr,cls){return{__class__member_descriptor,clscls,attrattr}}
)
...
$B.GenericAlias.__args__={__get__function(self){return $B.fast_tuple(self.items)}}
...
$B.GenericAlias.__or__=function(self,other){var $=$B.args('__or__',2,{selfnull,othernull},['self','other'],arguments,{},null,null)
return $B.UnionType.$factory([self,other])}
$B.GenericAlias.__origin__={__get__function(self){return self.origin_class}}
$B.GenericAlias.__parameters__={__get__function(self){
return $B.fast_tuple([])}}
...
$B.UnionType.__args__={__get__function(self){return $B.fast_tuple(self.items)}}
$B.UnionType.__parameters__={__get__function(){return $B.fast_tuple([])}}
...
if(iterator.__class__===_b_.range){var obj={ixiterator.start}
...
res.toString().search(ei)==-1){return res}else{res=eval(`${BigInt(left)}n ${op1} ${BigInt(right)}n`)
...
$B.stdin={__class__$io,__original__true,closedfalse,len1,pos0,readfunction(){return },readlinefunction(){return }}
$B.make_iterator_class=function(name){
var klass={__class___b_.type,__mro__[_b_.object],$factoryfunction(items){return{
__class__klass,__dict__$B.empty_dict(),counter-1,itemsitems,lenitems.length}},$infos{__name__name},$is_classtrue,__iter__function(self){self.counter=self.counter===undefined -1 self.counter
self.len=self.items.length
return self},__len__function(self){return self.items.length},__next__function(self){if(typeof self.test_change==function && self.test_change()){
...
item=$B.$JS2Py(item)}
return item}
throw _b_.StopIteration.$factory(StopIteration)},__reduce_ex__function(self,protocol){return $B.fast_tuple([_b_.iter,_b_.tuple.$factory([self.items])])}}
...
var expected_class={__complex___b_.complex,__float___b_.float,__index___b_.int,__int___b_.int}
...
$B.floordiv=function(x,y){var z=xy
...
var reversed_op={__lt____gt__,__le____ge__,__gt____lt__,__ge____le__}
...
$B.repr={enterfunction(obj){if(repr_stack.has(obj)){return true}else{repr_stack.add(obj)}},leavefunction(obj){repr_stack.delete(obj)}}})(__BRYTHON__)
;
;(function($B){
...
var TimSort=function(array,compare){var self={arrayarray,comparecompare,minGallopDEFAULT_MIN_GALLOPING,length array.length,tmpStorageLengthDEFAULT_TMP_STORAGE_LENGTH,stackLength0,runStartnull,runLengthnull,stackSize0,
pushRunfunction(runStart,runLength){this.runStart[this.stackSize]=runStart
this.runLength[this.stackSize]=runLength
this.stackSize+=1},
mergeRunsfunction(){while(this.stackSize  1){var n=this.stackSize-2
...
this.mergeAt(n)}},
forceMergeRunsfunction(){while(this.stackSize  1){var n=this.stackSize-2
if(n  0 && this.runLength[n-1] this.runLength[n+1]){n--}
this.mergeAt(n)}},
mergeAtfunction(i){var compare=this.compare,array=this.array,start1=this.runStart[i],length1=this.runLength[i],start2=this.runStart[i+1],length2=this.runLength[i+1]
...
mergeLowfunction(start1,length1,start2,length2){var compare=this.compare,array=this.array,tmp=this.tmp,i=0
...
mergeHighfunction(start1,length1,start2,length2){var compare=this.compare,array=this.array,tmp=this.tmp,i=0
...
for(var i=0;i  length2;i++){array[customCursor+i]=tmp[i]}}}}
if(self.length  2DEFAULT_TMP_STORAGE_LENGTH){self.tmpStorageLength=self.length  1}
...
var NoneType={$factoryfunction(){return None},$infos{__name__NoneType,__module__builtins},__bool__function(self){return False},__class___b_.type,__hash__function(self){return 0},__mro__[_b_.object],__repr__function(self){return 'None'},__str__function(self){return 'None'},$is_classtrue}
NoneType.__setattr__=function(self,attr){return no_set_attr(NoneType,attr)}
var None={__class__NoneType,}
...
__class__$B.long_int,valueobj.value,postrue}}else{return _b_.int.$factory(Math.abs(obj))}}
...
function anext(async_iterator,_default){var missing={},$=$B.args('anext',2,{async_iteratornull,_defaultnull},['async_iterator','_default'],arguments,{_defaultmissing},null,null)
...
f.$infos={__func__func,__name__func.$infos.__name__}
f.__get__=function(obj,cls){var method=function(){return f(cls,...arguments)}
method.__class__=$B.method
method.$infos={__self__cls,__func__f,__name__func.$infos.__name__,__qualname__cls.$infos.__name__+.+func.$infos.__name__}
return method}
...
function compile(){var $=$B.args('compile',6,{sourcenull,filenamenull,modenull,flagsnull,dont_inheritnull,optimizenull,_feature_versionnull},['source','filename','mode','flags','dont_inherit','optimize','_feature_version'],arguments,{flags0,dont_inheritfalse,optimize-1,_feature_version0},null,null)
...
var encoding_re=^[tf]#.coding[=][t]([-_.a-zA-Z0-9]+)
...
if(interactive && ! $.source.endsWith(n)){
var lines=$.source.split(n)
...
var enumerate=$B.make_class(enumerate,function(){var $ns=$B.args(enumerate,2,{iterablenull,startnull},['iterable','start'],arguments,{start0},null,null),_iter=iter($ns[iterable]),start=$ns[start]
return{
__class__enumerate,__name__'enumerate iterator',counterstart-1,iter_iter,startstart}}
)
...
const handler={getfunction(target,prop){console.log('get proxy attr',prop,target)
...
return undefined},setfunction(target,prop,value){_b_.dict.$setitem(target,prop,value)}}
return new Proxy(dict,handler)}
function $$eval(src,_globals,_locals){var $=$B.args(eval,4,{srcnull,globalsnull,localsnull,modenull},[src,globals,locals,mode],arguments,{globals_b_.None,locals_b_.None,modeeval},null,null),src=$.src,_globals=$.globals,_locals=$.locals,mode=$.mode
...
if(src.endsWith('n')){var exc=_b_.SyntaxError.$factory('')
var lines=src.split('n'),line=lines[lines.length-2]
...
var handler={getfunction(obj,prop){if(prop=='$lineno'){return lineno}else if(prop=='__file__'){return 'string'}
return obj[prop]},setfunction(obj,prop,value){if(['__file__','$lineno'].indexOf(prop)==-1){obj[prop]=value}}}
...
if(_locals.$jsobj){for(var key in _locals.$jsobj){exec_globals[key]=_locals.$jsobj[key]}}else{if(_locals.$jsobj){exec_locals=_locals.$jsobj}else{exec_locals=_locals.$jsobj={$dict_locals}}
...
try{var exec_func=new Function('$B','_b_','locals',local_name,global_name,js)}catch(err){console.log('errorn',js)
throw err}
...
var $=$B.args(exec,3,{srcnull,globalsnull,localsnull},[src,globals,locals],arguments,{globals_b_.None,locals_b_.None},null,null),src=$.src,globals=$.globals,locals=$.locals
...
return{
__class__filter,funcfunc,iterableiterable}}
)
...
function format(value,format_spec){var $=$B.args(format,2,{valuenull,format_specnull},[value,format_spec],arguments,{format_spec''},null,null)
...
var $=$B.args(getattr,3,{objnull,attrnull,_defaultnull},[obj,attr,_default],arguments,{_defaultmissing},null,null)
...
var klass=obj.__class__
var $test=false 
if($test){console.log($getattr,attr,'nobj',obj,'nklass',klass)}
...
console.log('nobj[attr]',obj[attr])}
...
f.$infos={__name__attr,__qualname__attr}
...
method.__class__=$B.method
method.$infos={__func__func,__name__attr,__self__self,__qualname__klass.$infos.__name__+.+attr}
...
if($test){console.log(attr_func is odga ,attr_func,attr_func===odga,'nobj[attr]',obj[attr])}
...
var $=$B.args('__import__',5,{namenull,globalsnull,localsnull,fromlistnull,levelnull},['name','globals','locals','fromlist','level'],arguments,{globalsNone,localsNone,fromlist_b_.tuple.$factory(),level0},null,null)
...
__class__iterator_class,getitemgetitem,lenlen,counter-1}}
)
...
__class__callable_iterator,funcfunc,sentinelsentinel}}
)
...
function iter(){
var $=$B.args('iter',1,{objnull},['obj'],arguments,{},'args','kw'),sentinel
...
return res}
var map=$B.make_class(map,function(){var $=$B.args('map',2,{funcnull,it1null},['func','it1'],arguments,{},'args',null),func=$B.$call($.func)
var iter_args=[$B.$iter($.it1)]
$.args.forEach(function(item){iter_args.push($B.$iter(item))})
var obj={__class__map,argsiter_args,funcfunc}
return obj}
)
...
__class__memoryview,objobj,
format'B',itemsize1,ndim1,shape_b_.tuple.$factory([_b_.len(obj)]),strides_b_.tuple.$factory([1]),suboffsets_b_.tuple.$factory([]),c_contiguoustrue,f_contiguoustrue,contiguoustrue}}else{throw _b_.TypeError.$factory(memoryview a bytes-like object +
...
memoryview.__getitem__=function(self,key){if(isinstance(key,_b_.int)){var start=keyself.itemsize
...
for(var j=1;j  4;j++){item+=coefself.obj.source[i+j]
coef=256}
res.push(item)}
return res}}}
...
var missing={},$=$B.args(next,2,{objnull,defnull},['obj','def'],arguments,{defmissing},null,null)
...
var NotImplemented={__class__NotImplementedType}
...
function pow(){var $=$B.args('pow',3,{xnull,ynull,modnull},['x','y','mod'],arguments,{modNone},null,null),x=$.x,y=$.y,z=$.mod
var klass=x.__class__ $B.get_class(x)
...
var end=(ks['end']===undefined ks['end']===None) 'n' ks['end'][0],sep=(ks['sep']===undefined ks['sep']===None) ' ' ks['sep'][0],file=ks['file']===undefined  $B.stdout ks['file'][0],args=$ns['args'],writer=$B.$getattr(file,'write')
var items=[]
...
$print.is_func=true
var property=$B.make_class(property,function(fget,fset,fdel,doc){var res={__class__property}
property.__init__(res,fget,fset,fdel,doc)
return res}
)
property.__init__=function(self,fget,fset,fdel,doc){var $=$B.args('__init__',5,{selfnull,fgetnull,fsetnull,fdelnull,docnull},['self','fget','fset','fdel','doc'],arguments,{fget_b_.None,fset_b_.None,fdel_b_.None,doc_b_.None},null,null),self=$.self,fget=$.fget,fset=$.fset,fdel=$.fdel,doc=$.doc
self.__doc__=doc 
self.$type=fget.$type
self.fget=fget
self.fset=fset
...
var res={__class__reversed,$counter _b_.len(seq),getterfunction(i){return $B.$call(method)(seq,i)}}
return res}
)
...
function round(){var $=$B.args('round',2,{numbernull,ndigitsnull},['number','ndigits'],arguments,{ndigitsNone},null,null),arg=$.number,n=$.ndigits===None  0 $.ndigits
...
var mult=Math.pow(10,n),x=argmult,floor=Math.floor(x),diff=Math.abs(x-floor),res
...
function setattr(){var $=$B.args('setattr',3,{objnull,attrnull,valuenull},['obj','attr','value'],arguments,{},null,null),obj=$.obj,attr=$.attr,value=$.value
...
can't set attributes of built-inextension type '+
obj.$infos.__name__+')}
obj[attr]=value
...
return None}
function sorted(){var $=$B.args('sorted',1,{iterablenull},['iterable'],arguments,{},null,'kw')
var _list=_b_.list.$factory(iter($.iterable)),args=[_list]
for(var i=1;i  arguments.length;i++){args.push(arguments[i])}
_b_.list.sort.apply(null,args)
return _list}
var staticmethod=$B.make_class(staticmethod,function(func){var f={$infosfunc.$infos,__get__function(){return func}}
f.__get__.__class__=$B.method_wrapper
f.__get__.$infos=func.$infos
return f}
)
$B.set_func_names(staticmethod,builtins)
function sum(iterable,start){var $=$B.args('sum',2,{iterablenull,startnull},['iterable','start'],arguments,{start0},null,null),iterable=$.iterable,start=$.start
...
return{
__class__$$super,__thisclass___type,__self_class__object_or_type}}
)
...
method.$infos={__self__self.__self_class__,__func__f,__name__attr,__module__module,__qualname__self.__thisclass__.$infos.__name__+.+attr}
return method}
throw $B.attr_error(attr,self)}
...
function vars(){var def={},$=$B.args('vars',1,{objnull},['obj'],arguments,{objdef},null,null)
...
$Reader.read=function(){var $=$B.args(read,2,{selfnull,sizenull},[self,size],arguments,{size-1},null,null),self=$.self,size=$B.$GetInt($.size)
if(self.closed===true){throw _b_.ValueError.$factory('IO operation on closed file')}
...
if(self.$lines===undefined){if(! self.$binary){self.$lines=self.$content.split(n)
if($B.last(self.$lines)==''){self.$lines.pop()}
self.$lines=self.$lines.map(x= x+'n')}else{var lines=[],pos=0,source=self.$content.source
while(pos  self.$length){var ix=source.indexOf(10,pos)
if(ix==-1){lines.push({__class___b_.bytes,sourcesource.slice(pos)})
break}else{lines.push({__class___b_.bytes,sourcesource.slice(pos,ix+1)})
pos=ix+1}}
self.$lines=lines}}}
$Reader.readline=function(self,size){var $=$B.args(readline,2,{selfnull,sizenull},[self,size],arguments,{size-1},null,null),self=$.self,size=$B.$GetInt($.size)
...
return _b_.bytes.$factory(rest)}else{var res={__class___b_.bytes,source self.$content.source.slice(self.$counter,ix+1)}
self.$counter=ix+1
return res}}else{if(self.$counter==self.$content.length){return ''}
var ix=self.$content.indexOf(n,self.$counter)
...
self.$lc+=1
return res}}}
$Reader.readlines=function(){var $=$B.args(readlines,2,{selfnull,hintnull},[self,hint],arguments,{hint-1},null,null),self=$.self,hint=$B.$GetInt($.hint)
var nb_read=0
if(self.closed===true){throw _b_.ValueError.$factory('IO operation on closed file')}
self.$lc=self.$lc===undefined -1 self.$lc
make_lines(self)
...
return lines}
$Reader.seek=function(self,offset,whence){if(self.closed===True){throw _b_.ValueError.$factory('IO operation on closed file')}
...
__class__$BufferedReader,$binarytrue,$contentcontent,$read_func$B.$getattr(content,'read')}}
)
...
var $TextIOWrapper=$B.make_class('_io.TextIOWrapper',function(){var $=$B.args(TextIOWrapper,6,{buffernull,encodingnull,errorsnull,newlinenull,line_bufferingnull,write_throughnull},[buffer,encoding,errors,newline,line_buffering,write_through],arguments,{encodingutf-8,errors_b_.None,newline_b_.None,line_buffering_b_.False,write_through_b_.False},null,null)
return{
__class__$TextIOWrapper,$content_b_.bytes.decode($.buffer.$content,$.encoding),encoding$.encoding,errors$.errors,newline$.newline}}
)
...
function $url_open(){
var $=$B.args('open',3,{filenull,modenull,encodingnull},['file','mode','encoding'],arguments,{mode'r',encoding'utf-8'},'args','kw'),file=$.file,mode=$.mode,encoding=$.encoding,result={}
if(encoding=='locale'){
encoding='utf-8'}
...
req.overrideMimeType('textplain;charset=x-user-defined')
...
var res={$binaryis_binary,$contentresult.content,$counter0,$encodingencoding,$lengthis_binary  result.content.source.length 
result.content.length,closedFalse,mode,namefile}
res.__class__=is_binary  $BufferedReader $TextIOWrapper
return res}else{throw _b_.TypeError.$factory(invalid argument for open() +
_b_.str.$factory(file))}}
var zip=$B.make_class(zip,function(){var res={__class__zip,items[]}
...
functioniterator(args){while(true){var line=[],flag=true
...
var Ellipsis={__class__ellipsis}
...
$B.Function={__class___b_.type,__code__{__class__FunctionCode,__name__'function code'},__globals__{__class__FunctionGlobals,__name__'function globals'},__mro__[_b_.object],$infos{__name__'function',__module__builtins},$is_classtrue}
...
console.log($B.last($B.frames_stack))}
method.$infos={__name__self.$infos.__name__,__qualname__$B.class_name(obj)+.+self.$infos.__name__,__self__obj,__func__self}
return method}
$B.Function.__getattribute__=function(self,attr){
if(self.$infos && self.$infos[attr]!==undefined){if(attr=='__code__'){var res={__class__code}
...
_b_[name].$infos={__module__'builtins',__name__orig_name,__qualname__orig_name}}}
catch(err){}}
_b_['open']=$url_open
_b_['print']=$print
_b_['super']=$$super
...
if(src){var lines=src.split(n),line=lines[parseInt(info[0])-1]
...
return{
__class__ traceback,$stackstack,excexc}}
)
...
var res={__class__frame,f_builtins {},
$stackstack.slice()}
...
if(_frame.length  4 && _frame[4].$infos !==undefined){res.f_code=_frame[4].$infos.__code__}else{res.f_code={co_nameco_name,co_filenamefilename}
...
$B._frame=frame 
var BaseException=_b_.BaseException={__class___b_.type,__bases__ [_b_.object],__mro__[_b_.object],args[],$infos{__name__BaseException,__module__builtins},$is_classtrue}
...
BaseException.$factory.$infos={__name__BaseException,__qualname__BaseException}
...
if(src !==undefined){var lines=src.split('n'),line=lines[frame[1].$lineno-1]
console.log('    '+line)}}
...
var $exc=(be_factory).replace(BaseExceptiong,name)
$exc=$exc.replace( placeholder,code)
_str[pos++]=_b_.+name+' = {__class___b_.type, '+
...
var js='nvar $ = $B.args(AttributeError, 1, {msg null, namenull, objnull}, '+
'[msg, name, obj], arguments, '+
'{msg _b_.None, name _b_.None, obj _b_.None}, , null);n'+
'err.args = $B.fast_tuple($.msg === _b_.None  []  [$.msg])n;'+
'err.name = $.namenerr.obj = $.objn'
...
return _b_.AttributeError.$factory({$natkw,kw{name,obj,msg}})}
var js='nvar $ = $B.args(NameError, 1, {namenull}, '+
'[name], arguments, '+
'{name _b_.None}, , null);n'+
'err.args = $B.fast_tuple($.name === _b_.None  []  [$.name])n;'+
'err.name = $.namen'
...
$B.name_error=function(name,obj){return _b_.NameError.$factory({$natkw,kw{name}})}
...
if(b.length==0){return a.lengthMOVE_COST}
...
var distance=result=b_indexMOVE_COST;
...
var suggestion_distance=252,suggestion=null
for(var item of dir){
var max_distance=(name.length+item.length+3)MOVE_COST6
...
trace+='n'
if(src){var lines=src.split('n'),line=lines[lineno-1]
if(line){trace+='    '+line.trim()+'n'}}}
return trace}
$B.show_error=function(err){if($B.debug  1){console.log(handle error,err.__class__,err.args)
console.log('stack',err.$stack)
console.log(err.stack)}
var trace=''
if(err.$stack && err.$stack.length  0){trace='Traceback (most recent call last)n'}
...
if(! err.$stack err.$stack.length==0){trace+=`  File ${filename}, line ${err.args[1][1]}n`+
`    ${line.trim()}n`}
...
;(function($B){var _b_=$B.builtins,None=_b_.None,range={__class___b_.type,__mro__[_b_.object],$infos{__module__builtins,__name__range},$is_classtrue,$nativetrue,$match_sequence_patterntrue,
$descriptors{starttrue,steptrue,stoptrue}}
...
var RangeIterator={__class___b_.type,__mro__[_b_.object],__iter__function(self){return self},__next__function(self){return _b_.next(self.obj)},$infos{__name__range_iterator,__module__builtins},$is_classtrue}
RangeIterator.$factory=function(obj){return{__class__RangeIterator,objobj}}
$B.set_func_names(RangeIterator,builtins)
range.__iter__=function(self){var res={__class__ range,startself.start,stopself.stop,stepself.step}
...
throw err}}}}
range.index=function(self,other){var $=$B.args(index,2,{selfnull,othernull},[self,other],arguments,{},null,null),self=$.self,other=$.other
...
' not in range')}}else{throw _b_.ValueError.$factory(_b_.str.$factory(other)+
 not in range)}}
range.$factory=function(){var $=$B.args(range,3,{startnull,stopnull,stepnull},[start,stop,step],arguments,{startnull,stopnull,stepnull},null,null),start=$.start,stop=$.stop,step=$.step,safe
...
return{__class__range,start0,stopstop,step1,$is_rangetrue,$safesafe}}
if(step===null){step=1}
...
typeof step==number)
return{__class__range,startstart,stopstop,stepstep,$is_rangetrue,$safesafe}}
$B.set_func_names(range,builtins)
var slice={__class___b_.type,__mro__[_b_.object],$infos{__module__builtins,__name__slice},$is_classtrue,$nativetrue,$descriptors{starttrue,steptrue,stoptrue}}
...
return{startstart,stopstop,stepstep}}
...
var $=$B.args(indices,2,{selfnull,lengthnull},[self,length],arguments,{},null,null)
...
slice.$factory=function(){var $=$B.args(slice,3,{startnull,stopnull,stepnull},[start,stop,step],arguments,{stopnull,stepnull},null,null),start,stop,step
if($.stop===null && $.step===null){start=_b_.None
stop=$.start
step=_b_.None}else{start=$.start
stop=$.stop
step=$.step===null  _b_.None $.step}
var res={__class__ slice,startstart,stopstop,stepstep}
conv_slice(res)
return res}
...
var bytearray={__class___b_.type,__mro__[_b_.object],$buffer_protocoltrue,$infos{__module__builtins,__name__bytearray},$is_classtrue}
...
var bytes={__class__ _b_.type,__mro__[_b_.object],$buffer_protocoltrue,$infos{__module__builtins,__name__bytes},$is_classtrue}
...
var hash=1
for(var i=0,len=self.source.length;i  len;i++){hash=(101hash+self.source[i])& 0xFFFFFFFF}
return hash}
...
bytes.__mul__=function(){var $=$B.args('__mul__',2,{selfnull,othernull},['self','other'],arguments,{},null,null),other=$B.PyNumber_Index($.other)
...
bytes.__new__=function(cls,source,encoding,errors){var $=$B.args(__new__,4,{clsnull,sourcenull,encodingnull,errorsnull},[cls,source,encoding,errors],arguments,{source[],encodingutf-8,errorsstrict},null,null)
return bytes.$new($.cls,$.source,$.encoding,$.errors)}
bytes.$new=function(cls,source,encoding,errors){
var self={__class__cls},int_list=[],pos=0
...
bytes.center=function(){var $=$B.args('center',3,{selfnull,widthnull,fillbytenull},['self','width','fillbyte'],arguments,{fillbytebytes.$factory([32])},null,null)
var diff=$.width-$.self.source.length
if(diff =0){return bytes.$factory($.self.source)}
var ljust=bytes.ljust($.self,$.self.source.length+Math.floor(diff2),$.fillbyte)
return bytes.rjust(ljust,$.width,$.fillbyte)}
bytes.count=function(){var $=$B.args('count',4,{selfnull,subnull,startnull,endnull},['self','sub','start','end'],arguments,{start0,end-1},null,null)
...
return n}
bytes.decode=function(self,encoding,errors){var $=$B.args(decode,3,{selfnull,encodingnull,errorsnull},[self,encoding,errors],arguments,{encodingutf-8,errorsstrict},null,null)
...
default}}
bytes.endswith=function(){var $=$B.args('endswith',4,{selfnull,suffixnull,startnull,endnull},['self','suffix','start','end'],arguments,{start-1,end-1},null,null)
...
bytes.expandtabs=function(){var $=$B.args('expandtabs',2,{selfnull,tabsizenull},['self','tabsize'],arguments,{tabsize8},null,null)
var tab_spaces=[]
...
bytes.find=function(self,sub){if(arguments.length !=2){var $=$B.args('find',4,{selfnull,subnull,startnull,endnull},['self','sub','start','end'],arguments,{start0,end-1},null,null),sub=$.sub,start=$.start,end=$.end}else{var start=0,end=-1}
...
if(found){return i}}
return-1}
bytes.fromhex=function(){var $=$B.args('fromhex',2,{clsnull,stringnull},['cls','string'],arguments,{},null,null),string=$.string.replace(sg,''),source=[]
...
bytes.hex=function(){
var $=$B.args('hex',3,{selfnull,sepnull,bytes_per_sepnull},['self','sep','bytes_per_sep'],arguments,{sep,bytes_per_sep1},null,null),self=$.self,sep=$.sep,bytes_per_sep=$.bytes_per_sep,res=,digits=0123456789abcdef,bps=bytes_per_sep,jstart=bps,len=self.source.length;
...
res+=digits[c & 0x0f]}
return res}
bytes.index=function(){var $=$B.args('rfind',4,{selfnull,subnull,startnull,endnull},['self','sub','start','end'],arguments,{start0,end-1},null,null)
...
return true}
bytes.join=function(){var $ns=$B.args('join',2,{selfnull,iterablenull},['self','iterable'],arguments,{}),self=$ns['self'],iterable=$ns['iterable']
...
bytes.ljust=function(){var $=$B.args('ljust',3,{selfnull,widthnull,fillbytenull},['self','width','fillbyte'],arguments,{fillbytebytes.$factory([32])},null,null)
...
bytes.partition=function(){var $=$B.args('partition',2,{selfnull,sepnull},['self','sep'],arguments,{},null,null)
...
bytes.removeprefix=function(){var $=$B.args(removeprefix,2,{selfnull,prefixnull},[self,prefix],arguments,{},null,null)
...
bytes.removesuffix=function(){var $=$B.args(removesuffix,2,{selfnull,prefixnull},[self,suffix],arguments,{},null,null)
...
return bytes.__getitem__($.self,_b_.slice.$factory(0,_b_.None))}
bytes.replace=function(){var $=$B.args('replace',4,{selfnull,oldnull,newnull,countnull},['self','old','new','count'],arguments,{count-1},null,null),res=[]
...
bytes.rfind=function(self,subbytes){if(arguments.length==2 && subbytes.__class__===bytes){var sub=subbytes,start=0,end=-1}else{var $=$B.args('rfind',4,{selfnull,subnull,startnull,endnull},['self','sub','start','end'],arguments,{start0,end-1},null,null),self=$.self,sub=$.sub,start=$.start,end=$.end}
if(typeof sub==number){if(sub  0 sub  255){throw _b_.ValueError.$factory(byte must be in range(0, 256))}
...
if(found){return i}}
return-1}
bytes.rindex=function(){var $=$B.args('rfind',4,{selfnull,subnull,startnull,endnull},['self','sub','start','end'],arguments,{start0,end-1},null,null)
var index=bytes.rfind($.self,$.sub,$.start,$.end)
if(index==-1){throw _b_.ValueError.$factory(subsection not found)}
return index}
bytes.rjust=function(){var $=$B.args('rjust',3,{selfnull,widthnull,fillbytenull},['self','width','fillbyte'],arguments,{fillbytebytes.$factory([32])},null,null)
if(!$.fillbyte.__class__){throw _b_.TypeError.$factory(argument 2 must be a byte string of length 1, +
not '+$B.class_name($.fillbyte)+')}else if(!$.fillbyte.__class__.$buffer_protocol){throw _b_.TypeError.$factory(argument 2 must be a byte string of length 1, +
not '+$B.class_name($.fillbyte)+')}
var padding=[],count=$.width-$.self.source.length
...
bytes.rpartition=function(){var $=$B.args('rpartition',2,{selfnull,sepnull},['self','sep'],arguments,{},null,null)
if(!$.sep.__class__){throw _b_.TypeError.$factory(a bytes-like object is required, +
not '+$B.class_name($.sep)+')}else if(!$.sep.__class__.$buffer_protocol){throw _b_.TypeError.$factory(a bytes-like object is required, +
not '+$B.class_name($.sep)+')}
...
bytes.split=function(){var $=$B.args('split',2,{selfnull,sepnull},['self','sep'],arguments,{sepbytes.$factory([32])},null,null),res=[],start=0,stop=0
if(! $.sep.__class__ ){throw _b_.TypeError.$factory(a bytes-like object is required, +
not '+$B.class_name($.sep)+')}else if(! $.sep.__class__.$buffer_protocol){throw _b_.TypeError.$factory(a bytes-like object is required, +
not '+$B.class_name($.sep)+')}
...
return res}
bytes.splitlines=function(self){var $=$B.args('splitlines',2,{selfnull,keependsnull},['self','keepends'],arguments,{keependsfalse},null,null)
...
return res}
bytes.startswith=function(){var $=$B.args('startswith',3,{selfnull,prefixnull,startnull},['self','prefix','start'],arguments,{start0},null,null),start=$.start
...
return s}
var encode=$B.encode=function(){var $=$B.args(encode,3,{snull,encodingnull,errorsnull},[s,encoding,errors],arguments,{encodingutf-8,errorsstrict},null,null),s=$.s,encoding=$.encoding,errors=$.errors
...
return t}
bytes.$factory=function(source,encoding,errors){var $=$B.args(bytes,3,{sourcenull,encodingnull,errorsnull},[source,encoding,errors],arguments,{source[],encodingutf-8,errorsstrict},null,null)
...
return res}
var set={__class___b_.type,$infos{__module__builtins,__name__set},$is_classtrue,$nativetrue}
...
return $N}}
var $=$B.args(__init__,2,{selfnull,iterablenull},[self,iterable],arguments,{iterable[]},null,null),self=$.self,iterable=$.iterable
...
return $N}
set.add=function(){var $=$B.args(add,2,{selfnull,itemnull},[self,item],arguments,{},null,null),self=$.self,item=$.item
return $add(self,item)}
set.clear=function(){var $=$B.args(clear,1,{selfnull},[self],arguments,{},null,null)
$.self.$items=[]
$.self.$numbers=[]
$.self.$hashes={}
return $N}
set.copy=function(){var $=$B.args(copy,1,{selfnull},[self],arguments,{},null,null)
...
return res}
set.difference_update=function(self){var $=$B.args(difference_update,1,{selfnull},[self],arguments,{},args,null)
...
throw err}}}
return $N}
set.discard=function(){var $=$B.args(discard,2,{selfnull,itemnull},[self,item],arguments,{},null,null)
...
var $=$B.args(intersection_update,1,{selfnull},[self],arguments,{},args,null),self=$.self
...
return $N}
set.isdisjoint=function(){var $=$B.args(is_disjoint,2,{selfnull,othernull},[self,other],arguments,{},null,null)
...
var $=$B.args(remove,2,{selfnull,itemnull},[self,item],arguments,{},null,null),self=$.self,item=$.item
...
var $=$B.args(symmetric_difference_update,2,{selfnull,snull},[self,s],arguments,{},null,null),self=$.self,s=$.s
...
var $=$B.args(update,1,{selfnull},[self],arguments,{},args,null)
...
return $N}
set.difference=function(){var $=$B.args(difference,1,{selfnull},[self],arguments,{},args,null)
...
fc.replace(differenceg,intersection).replace(__sub__,__and__))
eval(set.symmetric_difference = +
fc.replace(differenceg,symmetric_difference).replace(__sub__,__xor__))
eval(set.union = +
fc.replace(differenceg,union).replace(__sub__,__or__))
set.issubset=function(){var $=$B.args(issubset,2,{selfnull,othernull},[self,other],arguments,{},args,null),func=_b_.getattr($.other,__contains__)
for(var i=0,len=$.self.$items.length;i  len;i++){if(! func($.self.$items[i])){return false}}
return true}
set.issuperset=function(){var $=$B.args(issuperset,2,{selfnull,othernull},[self,other],arguments,{},args,null)
...
var res={__class__set,$simpletrue,$items[],$numbers[],$hashes{}}
...
var frozenset={__class___b_.type,__mro__[object],$infos{__module__builtins,__name__frozenset},$is_classtrue,$nativetrue}
...
return{
__class__cls,$simpletrue,$items[],$numbers[],$hashes{}}}
...
res.$id=singleton_id
return res}
frozenset.$factory=function(){var $=$B.args(frozenset,1,{iterablenull},[iterable],arguments,{iterablenull},null,null)
...
var JSConstructor={__class___b_.type,__mro__[object],$infos{__module__javascript,__name__'JSConstructor'},$is_classtrue}
...
__class__JSConstructor,jsobj,funcobj.js_func}}
...
res.$infos={__name__js_attr.name,__qualname__js_attr.name}
...
__class__cls,__name__name,__doc__doc _b_.None,__package__$package _b_.None}}
...
try{var $module=new Function(module_id,module_contents+;nreturn $module)(_module)
...
for(var attr in $module){if(typeof $module[attr]==function){$module[attr].$infos={__module___module.__name__,__name__attr,__qualname__attr}
...
var src={srcmodule_contents,has_annotationsfalse,filenamepath}
...
var src=js
js=var $module = (function(){n+js
var prefix='locals_'
js+='return '+prefix
js+=module.__name__.replace(.g,_)+})(__BRYTHON__)n+
return $module
...
return{
contentsrc,namemod_name,importsObject.keys(root.imports).join(,)}}catch(err){console.log(+err+ + for module +module.__name__)
for(var attr in err){console.log(attr+ +err[attr])}
...
return ModuleSpec.$factory({name fullname,loaderVFSLoader.$factory(),
origin is_builtin built-in brython_stdlib,
submodule_search_locationsis_package[]_b_.None,loader_state{storedstored,timestamptimestamp},
cached_b_.None,parentis_package fullname parent_package(fullname),has_location_b_.False})}}
...
var finder_cpython={__class___b_.type,__mro__[_b_.object],$infos{__module__builtins,__name__CPythonFinder},create_module function(cls,spec){
...
run_py(content,modobj.__path__,modobj)},find_modulefunction(cls,name,path){return{
__class__Loader,load_modulefunction(name,path){var spec=cls.find_spec(cls,name,path)
...
var xhr=new XMLHttpRequest(),url=cpython_importmodule=+fullname,result
xhr.open(GET,url,false)
xhr.onreadystatechange=function(){if(this.readyState==4 && this.status==200){var data=JSON.parse(this.responseText)
result=ModuleSpec.$factory({name fullname,loadercls,
origin CPython,
submodule_search_locationsdata.is_package[]_b_.None,loader_state{contentdata.content},
cached_b_.None,parentdata.is_package fullname parent_package(fullname),has_location_b_.False})}}
xhr.send()
return result}}
...
finder_cpython.$factory=function(){return{__class__finder_cpython}}
var StdlibStaticFinder=$B.make_class(StdlibStaticFinder,function(){return{
__class__StdlibStaticFinder}}
)
...
((ext==py) Lib libs)+
fullname.replace(.g,),metadata={extext,is_packageis_pkg,pathpath+(is_pkg __init__.py 
((ext==py) .py .js)),addressaddress},_module=Module.$factory(fullname)
metadata.code=$download_module(_module,metadata.path)
var res=ModuleSpec.$factory({name fullname,loaderPathLoader.$factory(),
origin metadata.path,submodule_search_locationsis_pkg[path]_b_.None,loader_statemetadata,
cached_b_.None,parentis_pkg  fullname parent_package(fullname),has_location_b_.True})
return res}}
return _b_.None}
...
StdlibStaticFinder.$factory=function(){return{__class__StdlibStaticFinder}}
...
__class__PathEntryFinder,path_entrypath_entry,hinthint}}
)
...
for(var j=0;notfound && j  modpaths.length;++j){try{var file_info=modpaths[j],module={__name__fullname,$is_packagefalse}
...
if(!notfound){return ModuleSpec.$factory({name fullname,loaderPathLoader.$factory(),origin loader_data.path,
submodule_search_locationsloader_data.is_package
[base_path]_b_.None,loader_stateloader_data,
cached_b_.None,parentloader_data.is_package fullname 
parent_package(fullname),has_location_b_.True})}
return _b_.None}
...
! file.startsWith($B.brython_path+Libsite-packages))
...
$B.finders={VFSVFSFinder,stdlib_staticStdlibStaticFinder,pathPathFinder,CPythonfinder_cpython}
...
var _importlib_module={__class__ Module,__name__ _importlib,LoaderLoader,VFSFinderVFSFinder,StdlibStaticStdlibStaticFinder,ImporterPathPathFinder,UrlPathFinderurl_hook,optimize_import_for_path optimize_import_for_path}
...
for(var i=0,nb=item[1];i  nb;i+=1){$B.unicode_tables[gc][item[0]+istep]=true}}else{$B.unicode_tables[gc][item]=true}})}
...
return jspos-nb}
var str={__class___b_.type,__dir___b_.object.__dir__,$infos{__module__builtins,__name__str},$is_classtrue,$nativetrue}
...
var prefix=2,suffix=3,mask=(232-1),str_hash_cache={}
str.$nb_str_hash_cache=0
function fnv(p){if(p.length==0){return 0}
var x=prefix
x=(x ^(p[0] 7))& mask
for(var i=0,len=p.length;i  len;i++){x=((1000003x)^ p[i])& mask}
x=(x ^ p.length)& mask
x=(x ^ suffix)& mask
if(x==-1){x=-2}
return x}
...
var char_mapping={bseries_of_bytes,sstr_format,dnum_format,inum_format,unum_format,ooctal_format,rrepr_format,aascii_format,gfunction(val,flags){return floating_point_format(val,false,flags)},Gfunction(val,flags){return floating_point_format(val,true,flags)},ffunction(val,flags){return floating_point_decimal_format(val,false,flags)},Ffunction(val,flags){return floating_point_decimal_format(val,true,flags)},efunction(val,flags){return floating_point_exponential_format(val,false,flags)},Efunction(val,flags){return floating_point_exponential_format(val,true,flags)},xfunction(val,flags){return signed_hex_format(val,false,flags)},Xfunction(val,flags){return signed_hex_format(val,true,flags)},csingle_char_format,0function(val,flags){return num_flag(0,flags)},1function(val,flags){return num_flag(1,flags)},2function(val,flags){return num_flag(2,flags)},3function(val,flags){return num_flag(3,flags)},4function(val,flags){return num_flag(4,flags)},5function(val,flags){return num_flag(5,flags)},6function(val,flags){return num_flag(6,flags)},7function(val,flags){return num_flag(7,flags)},8function(val,flags){return num_flag(8,flags)},9function(val,flags){return num_flag(9,flags)},-neg_flag, space_flag,+sign_flag,.decimal_point_flag,#alternate_flag}
...
str.__mul__=function(){var $=$B.args(__mul__,2,{selfnull,othernull},[self,other],arguments,{},null,null)
...
repl+='x'+cp}else if(cp =0x300 && cp =0x36F){repl+=u200B+chars[i]+' '}else if(cp.toString(16)=='feff'){repl+='ufeff'}else{repl+=chars[i]}}
var res=repl
if(res.search('')==-1 && res.search(')==-1){return '+res+'}else if(self.search('')==-1){return ''+res+''}
var qesc=new RegExp(',g)
res='+res.replace(qesc,')+'
return res}
str.__rmod__=function(){var $=$B.args('__rmod__',2,{selfnull,othernull},['self','other'],arguments,{},null,null)
...
if(chars.length==self.length){return self.replace(combining_re,u200B$1)}
for(var i=0;i  chars.length;i++){var cp=_b_.ord(chars[i])
if(cp =0x300 && cp =0x36F){repl+=u200B+chars[i]}else{repl+=chars[i]}}
return repl}
...
return res}
str.center=function(){var $=$B.args(center,3,{selfnull,widthnull,fillcharnull},[self,width,fillchar],arguments,{fillchar },null,null),self=$.self
if($.width =self.length){return self}
var pad=parseInt(($.width-self.length)2),res=$.fillchar.repeat(pad)
res+=self+res
if(res.length  $.width){res+=$.fillchar}
return res}
str.count=function(){var $=$B.args(count,4,{selfnull,subnull,startnull,stopnull},[self,sub,start,stop],arguments,{startnull,stopnull},null,null)
...
return n}
str.encode=function(){var $=$B.args(encode,3,{selfnull,encodingnull,errorsnull},[self,encoding,errors],arguments,{encodingutf-8,errorsstrict},null,null)
...
var $=$B.args(endswith,4,{selfnull,suffixnull,startnull,endnull},[self,suffix,start,end],arguments,{start0,endnull},null,null)
normalize_start_end($)
var suffixes=$.suffix
...
return false}
str.expandtabs=function(self,tabsize){var $=$B.args(expandtabs,2,{selfnull,tabsizenull},[self,tabsize],arguments,{tabsize8},null,null)
var s=$B.$GetInt($.tabsize),col=0,pos=0,res=,chars=to_chars(self)
if(s==1){return self.replace(tg, )}
...
return res}
str.find=function(){
var $=$B.args(str.find,4,{selfnull,subnull,startnull,endnull},[self,sub,start,end],arguments,{start0,endnull},null,null)
check_str($.sub)
normalize_start_end($)
...
return{namename,name_extname_ext,convconv,specspec ,stringfmt_string}}
...
var $=$B.args(format,1,{selfnull},[self],args,{},$args,null)}else{var $=$B.args(format,1,{selfnull},[self],arguments,{},$args,$kw),mapping=$.$kw,
...
return res}
str.format_map=function(self,mapping){var $=$B.args(format_map,2,{selfnull,mappingnull},['self','mapping'],arguments,{},null,null)
...
var $=$B.args(isalnum,1,{selfnull},[self],arguments,{},null,null),cp
...
var $=$B.args(isalpha,1,{selfnull},[self],arguments,{},null,null),cp
for(var char of to_chars(self)){cp=_b_.ord(char)
if(unicode_tables.Ll[cp]
unicode_tables.Lu[cp]
unicode_tables.Lm[cp]
...
var $=$B.args(isdecimal,1,{selfnull},[self],arguments,{},null,null),cp
for(var char of to_chars(self)){cp=_b_.ord(char)
if(! unicode_tables.Nd[cp]){return false}}
return self.length  0}
str.isdigit=function(self){
var $=$B.args(isdigit,1,{selfnull},[self],arguments,{},null,null),cp
for(var char of to_chars(self)){cp=_b_.ord(char)
if(! unicode_tables.digits[cp]){return false}}
return self.length  0}
str.isidentifier=function(self){
var $=$B.args(isidentifier,1,{selfnull},[self],arguments,{},null,null)
...
var $=$B.args(islower,1,{selfnull},[self],arguments,{},null,null),has_cased=false,cp
for(var char of to_chars(self)){cp=_b_.ord(char)
if(unicode_tables.Ll[cp]){has_cased=true
continue}else if(unicode_tables.Lu[cp]unicode_tables.Lt[cp]){return false}}
return has_cased}
str.isnumeric=function(self){
var $=$B.args(isnumeric,1,{selfnull},[self],arguments,{},null,null)
...
unprintable[32]=true}
var $=$B.args(isprintable,1,{selfnull},[self],arguments,{},null,null)
for(var char of to_chars(self)){if(unprintable[_b_.ord(char)]){return false}}
return true}
str.isspace=function(self){
var $=$B.args(isspace,1,{selfnull},[self],arguments,{},null,null),cp
for(var char of to_chars(self)){cp=_b_.ord(char)
if(! unicode_tables.Zs[cp]&&
$B.unicode_bidi_whitespace.indexOf(cp)==-1){return false}}
return self.length  0}
str.istitle=function(self){
var $=$B.args(istitle,1,{selfnull},[self],arguments,{},null,null)
return self.length  0 && str.title(self)==self}
str.isupper=function(self){
var $=$B.args(islower,1,{selfnull},[self],arguments,{},null,null),is_upper=false,cp
for(var char of to_chars(self)){cp=_b_.ord(char)
if(unicode_tables.Lu[cp]){is_upper=true
continue}else if(unicode_tables.Ll[cp]unicode_tables.Lt[cp]){return false}}
return is_upper}
str.join=function(){var $=$B.args(join,2,{selfnull,iterablenull},[self,iterable],arguments,{},null,null)
...
str.ljust=function(self){var $=$B.args(ljust,3,{selfnull,widthnull,fillcharnull},[self,width,fillchar],arguments,{fillchar },null,null),len=str.__len__(self)
if($.width =len){return self}
return self+$.fillchar.repeat($.width-len)}
str.lower=function(self){var $=$B.args(lower,1,{selfnull},[self],arguments,{},null,null)
return self.toLowerCase()}
str.lstrip=function(self,x){var $=$B.args(lstrip,2,{selfnull,charsnull},[self,chars],arguments,{chars_b_.None},null,null),self=$.self,chars=$.chars
...
return ''}
str.maketrans=function(){var $=$B.args(maketrans,3,{xnull,ynull,znull},[x,y,z],arguments,{ynull,znull},null,null)
var _t=$B.empty_dict()
if($.y===null && $.z===null){
...
str.partition=function(){var $=$B.args(partition,2,{selfnull,sepnull},[self,sep],arguments,{},null,null)
...
str.removeprefix=function(){var $=$B.args(removeprefix,2,{selfnull,prefixnull},[self,prefix],arguments,{},null,null)
...
str.removesuffix=function(){var $=$B.args(removesuffix,2,{selfnull,prefixnull},[self,suffix],arguments,{},null,null)
...
var $=$B.args(replace,4,{selfnull,oldnull,newnull,countnull},[self,old,new,count],arguments,{count-1},null,null),count=$.count,self=$.self,old=$.old,_new=$.new
...
var $=$B.args(rfind,4,{selfnull,subnull,startnull,endnull},[self,sub,start,end],arguments,{start0,endnull},null,null)
normalize_start_end($)
check_str($.sub)
var len=str.__len__($.self),sub_len=str.__len__($.sub)
...
if(res==-1){throw _b_.ValueError.$factory(substring not found)}
return res}
str.rjust=function(self){var $=$B.args(rjust,3,{selfnull,widthnull,fillcharnull},[self,width,fillchar],arguments,{fillchar },null,null)
var len=str.__len__(self)
if($.width =len){return self}
return $B.String($.fillchar.repeat($.width-len)+self)}
str.rpartition=function(self,sep){var $=$B.args(rpartition,2,{selfnull,sepnull},[self,sep],arguments,{},null,null)
check_str($.sep)
var self=reverse($.self),sep=reverse($.sep)
var items=str.partition(self,sep).reverse()
for(var i=0;i  items.length;i++){items[i]=items[i].split().reverse().join()}
return items}
str.rsplit=function(self){var $=$B.args(rsplit,3,{selfnull,sepnull,maxsplitnull},[self,sep,maxsplit],arguments,{sep_b_.None,maxsplit-1},null,null),sep=$.sep
...
return rev_res}
str.rstrip=function(self,x){var $=$B.args(rstrip,2,{selfnull,charsnull},[self,chars],arguments,{chars_b_.None},null,null),self=$.self,chars=$.chars
...
if(! flag){return $.self.surrogates  $B.String(self)self}}
return ''}
str.split=function(){var $=$B.args(split,3,{selfnull,sepnull,maxsplitnull},[self,sep,maxsplit],arguments,{sep_b_.None,maxsplit-1},null,null),sep=$.sep,maxsplit=$.maxsplit,self=$.self,pos=0
...
res.push(s)
return res.map($B.String)}}
str.splitlines=function(self){var $=$B.args('splitlines',2,{selfnull,keependsnull},['self','keepends'],arguments,{keependsfalse},null,null)
if(!_b_.isinstance($.keepends,[_b_.bool,_b_.int])){throw _b_.TypeError('integer argument expected, got '+
$B.get_class($.keepends).__name)}
var keepends=_b_.int.$factory($.keepends),res=[],self=$.self,start=0,pos=0
if(!self.length){return res}
while(pos  self.length){if(self.substr(pos,2)=='rn'){res.push(self.slice(start,keepends  pos+2 pos))
start=pos=pos+2}else if(self[pos]=='r' self[pos]=='n'){res.push(self.slice(start,keepends  pos+1 pos))
start=pos=pos+1}else{pos++}}
if(start  self.length){res.push(self.slice(start))}
return res.map($B.String)}
str.startswith=function(){
var $=$B.args(startswith,4,{selfnull,prefixnull,startnull,endnull},[self,prefix,start,end],arguments,{start0,endnull},null,null)
normalize_start_end($)
var prefixes=$.prefix
...
return false}
str.strip=function(){var $=$B.args(strip,2,{selfnull,charsnull},[self,chars],arguments,{chars_b_.None},null,null)
...
return res.join()}
str.upper=function(self){var $=$B.args(upper,1,{selfnull},[self],arguments,{},null,null)
return self.toUpperCase()}
str.zfill=function(self,width){var $=$B.args(zfill,2,{selfnull,widthnull},[self,width],arguments,{},null,null),len=str.__len__(self)
if($.width =len){return self}
...
var $=$B.args(str,3,{argnull,encodingnull,errorsnull},[arg,encoding,errors],arguments,{encodingutf-8,errorsstrict},null,null),encoding=$.encoding,errors=$.errors}
...
var StringSubclass=$B.StringSubclass={__class___b_.type,__mro__[_b_.object],$infos{__module__builtins,__name__str},$is_classtrue}
...
StringSubclass.__new__=function(cls){return{__class__cls}}
...
var int={__class___b_.type,__dir___b_.object.__dir__,__mro__[_b_.object],$infos{__module__builtins,__name__int},$is_classtrue,$nativetrue,$descriptors{numeratortrue,denominatortrue,imagtrue,realtrue}}
int.as_integer_ratio=function(){var $=$B.args(as_integer_ratio,1,{selfnull},[self],arguments,{},null,null)
return $B.$list([$.self,1])}
int.from_bytes=function(){var $=$B.args(from_bytes,3,{bytesnull,byteordernull,signednull},[bytes,byteorder,signed],arguments,{signedfalse},null,null)
...
int.to_bytes=function(){var $=$B.args(to_bytes,3,{selfnull,lennull,byteordernull,signednull},[self,len,byteorder,,signed],arguments,{signedfalse},null,null),self=$.self,len=$.len,byteorder=$.byteorder,signed=$.signed
...
var res=[],value=self
while(value  0){var quotient=Math.floor(value256),rest=value-256quotient
res.push(rest)
...
if(fmt.comma){var sign=res[0]==-  - ,rest=res.substr(sign.length),len=rest.length,nb=Math.ceil(rest.length3),chunks=[]
for(var i=0;i  nb;i++){chunks.push(rest.substring(len-3i-3,len-3i))}
chunks.reverse()
...
while(exponent  0){if(exponent % 2==1){if(resultbase  $B.max_int){result=long_int.__mul__(
long_int.$factory(result),long_int.$factory(base))
result=long_int.__mod__(result,z)}else{result=(resultbase)% z}}
exponent=exponent  1
if(basebase  $B.max_int){base=long_int.__mul__(long_int.$factory(base),long_int.$factory(base))
base=long_int.__mod__(base,z)}else{base=(basebase)% z}}
return result}
...
__class__$B.long_int,value($B.BigInt(self)$B.BigInt(other)).toString(),postrue}}
...
return $B.make_complex(prealMath.cos(ln),prealMath.sin(ln))}
...
if(other==0){throw _b_.ZeroDivisionError.$factory(division by zero)}
if(other.__class__===$B.long_int){return new Number(selfparseInt(other.value))}
return new Number(selfother)}
return _b_.NotImplemented}
...
replace(__gt__gm,__+$B.$comps[$op]+__).
...
var $ns=$B.args(int,2,{xnull,basenull},[x,base],arguments,{base10},null,null),value=$ns[x],base=$ns[base]
...
if(match===null){invalid(value,base)}else{value=_value.replace(_g,)}
...
return res}}}
var bool={__bases__[int],__class___b_.type,__mro__[int,_b_.object],$infos{__name__bool,__module__builtins},$is_classtrue,$nativetrue,$descriptors{numeratortrue,denominatortrue,imagtrue,realtrue}}
...
var $=$B.args(bool,1,{xnull},[x],arguments,{xfalse},null,null)
return $B.$bool($.x)}
...
var long_int={__class___b_.type,__mro__[_b_.int,_b_.object],$infos{__module__builtins,__name__int},$is_classtrue,$nativetrue,$descriptors{numeratortrue,denominatortrue,imagtrue,realtrue}}
var max_safe_divider=$B.max_int9
function add_pos(v1,v2){
return{
__class__long_int,value(BigInt(v1)+BigInt(v2)).toString(),postrue}}
var len=((Math.pow(2,53)-1)+'').length-1
function binary_pos(t){var nb_chunks=Math.ceil(t.lengthlen),chunks=[],pos,start,nb,bin=[]
...
var res=''
for(var i=0,len=bpos.length;i  len;i++){res+=bpos.charAt(i)==0  10}
...
if(n==1){return[t,0]}
var quotient=BigInt(t)BigInt(n),rest=BigInt(t)-quotientBigInt(n)
console.log(divmod by safe int)
return[from_BigInt(quotient),from_BigInt(rest)]}
function divmod_pos(v1,v2){
var a={__class__long_int,value(BigInt(v1)BigInt(v2)).toString(),postrue},b={__class__long_int,value(BigInt(v1)% BigInt(v2)).toString(),postrue}
return[a,b]}
function split_chunks(s,size){var nb=Math.ceil(s.lengthsize),chunks=[],len=s.length
...
return intOrLong({__class__long_int,valuey,pospos})}
...
return{__class__long_int,valuev,posvalue =0}}
function preformat(self,fmt){if(fmt.empty){return _b_.str.$factory(self)}
if(fmt.type && 'bcdoxXn'.indexOf(fmt.type)==-1){throw _b_.ValueError.$factory(Unknown format code '+fmt.type+
' for object of type 'int')}
var res
...
res=sign+chunks.join(,)}
return $B.format_width(res,fmt)}
long_int.__abs__=function(self){return{__class__long_int,valueself.value,postrue}}
...
if((a =0 && b  0)(a =0 && b  0)){quotient=ab}else{quotient=ab-BigInt(1)}
var rest=a-quotientb
...
return{__class__long_int,valueres[0],pospos}}
...
return intOrLong({__class__long_int,value(BigInt(self.value) BigInt(shift)).toString(),posself.pos})}
...
return res===_b_.NotImplemented  res !res}
long_int.__neg__=function(obj){return{__class__long_int,valueobj.value,pos! obj.pos}}
...
__class__long_int,value(sx).toString(),postrue}}
while(x  0){if(x % $B.BigInt(2)==1){b=bs}
x=x$B.BigInt(2)
if(x  0){s=ss}
if(z !==undefined){b=b % z}}
return{__class__long_int,valueb.toString(),postrue}}
long_int.__rshift__=function(self,shift){if(shift.__class__===long_int){shift=shift.value}
return intOrLong(
{__class__long_int,value(BigInt(self.value) BigInt(shift)).toString(),posself.pos}
)}
...
break
case 0
res={__class__long_int,value0,postrue}
break
case-1
...
var nbits=$B.long_int.bit_length(self),pow2=2nBigInt(nbits-1),rest=BigInt(self.value)-pow2,relative_rest=new Number(rest)new Number(pow2)
...
Math.log(1+infos.relative_restMath.LN2))}
...
return long}
long_int.$from_int=function(value){return{__class__long_int,valuevalue.toString(),posvalue  0}}
...
return v10}
return{__class__long_int,valuevalue,pospos}}
...
$B.long_int=long_int
$B.fast_long_int=function(value,pos){return{__class__$B.long_int,valuevalue,pospos}}})(__BRYTHON__)
;
;(function($B){var _b_=$B.builtins
var object=_b_.object
function $err(op,other){var msg=unsupported operand type(s) for +op+
...
return obj.$brython_value !==undefined  obj.$brython_value obj}
var float={__class___b_.type,__dir__object.__dir__,$infos{__module__builtins,__name__float},$is_classtrue,$nativetrue,$descriptors{numeratortrue,denominatortrue,imagtrue,realtrue}}
...
return float.$factory(Math.floor(selfother))}
return _b_.NotImplemented}
...
return new Number(_sign_sumMath.pow(2,parseInt(_exponent.substring(1))))}
...
if(missing  0){res+=0.repeat(missing)}}}else if(fmt.type && fmt.type.toLowerCase()==g){var exp_fmt=preformat(self,{typee}).split(e),exp=parseInt(exp_fmt[1])
if(-4 =exp && exp  fmt.precision){res=preformat(self,{typef,precisionfmt.precision-1-exp})}else{res=preformat(self,{typee,precisionfmt.precision-1})}
...
if(fmt.comma){var len=_int.length,nb=Math.ceil(_int.length3),chunks=[]
for(var i=0;i  nb;i++){chunks.push(_int.substring(len-3i-3,len-3i))}
chunks.reverse()
raw[0]=chunks.join(,)}
...
if(_b_.isinstance(other,float)){
var q=Math.floor(selfother),r=self-otherq
return new Number(r)}
return _b_.NotImplemented}
float.__mro__=[object]
float.__mul__=function(self,other){self=float_value(self)
other=float_value(other)
if(_b_.isinstance(other,_b_.int)){if(other.__class__==$B.long_int){return new Number(selfparseFloat(other.value))}
other=_b_.int.numerator(other)
return new Number(selfother)}
if(_b_.isinstance(other,float)){return new Number(selffloat_value(other))}
return _b_.NotImplemented}
...
if(self==Infinity){return 'inf'}else if(self==-Infinity){return '-inf'}else if(isNaN(self)){return 'nan'}else if(self===0){if(1self===-Infinity){return '-0.0'}
return '0.0'}
var res=self+ 
if(res.search([.eE])==-1){res+=.0}
var split_e=res.split(ei)
...
other=float_value(other)
if(_b_.isinstance(other,[_b_.int,float])){if(other.valueOf()==0){throw _b_.ZeroDivisionError.$factory(division by zero)}
return float.$factory(selfother)}
return _b_.NotImplemented}
...
var $ops={+add,-sub}
for(var $op in $ops){var $opf=$op_func.replace(-gm,$op)
$opf=$opf.replace(__rsub__gm,__r+$ops[$op]+__)
...
var arabic_digits=u0660u0661u0662u0663u0664u0665u0666u0667u0668u0669,res=
...
default
value=value.charAt(0)+value.substr(1).replace(_g,)
...
var FloatSubclass=$B.FloatSubclass={__class___b_.type,__mro__[object],$infos{__module__builtins,__name__float},$is_classtrue}
...
var complex={__class___b_.type,__dir___b_.object.__dir__,$infos{__module__builtins,__name__complex},$is_classtrue,$nativetrue,$descriptors{realtrue,imagtrue}}
...
return self.$imag1000003+self.$real}
complex.__init__=function(){return _b_.None}
complex.__invert__=function(self){return ~self}
complex.__mro__=[_b_.object]
complex.__mul__=function(self,other){if(_b_.isinstance(other,complex)){return make_complex(self.$realother.$real-self.$imagother.$imag,self.$imagother.$real+self.$realother.$imag)}else if(_b_.isinstance(other,_b_.int)){return make_complex(self.$realother.valueOf(),self.$imagother.valueOf())}else if(_b_.isinstance(other,_b_.float)){return make_complex(self.$realother,self.$imagother)}else if(_b_.isinstance(other,_b_.bool)){if(other.valueOf()){return self}
...
var res,missing={},args=$B.args(complex,3,{clsnull,realnull,imagnull},[cls,real,imag],arguments,{real0,imagmissing},null,null),$real=args.real,$imag=args.imag
...
function to_num(s){var res=parseFloat(s.charAt(0)+s.substr(1).replace(_g,))
...
$imag=0}
res={__class__complex,$real$real 0,$imag$imag 0}
return res}}
...
_b_.isinstance($imag,[_b_.float,_b_.int])){res={__class__complex,$real$real,$imag$imag}
return res}
...
function complex2expo(cx){var norm=Math.sqrt((cx.$realcx.$real)+(cx.$imagcx.$imag)),sin=cx.$imagnorm,cos=cx.$realnorm,angle
if(cos==0){angle=sin==1  Math.PI2 3Math.PI2}
else if(sin==0){angle=cos==1  0 Math.PI}
else{angle=Math.atan(sincos)}
return{normnorm,angleangle}}
...
a.$realb.$real-a.$imagb.$imag,a.$realb.$imag+a.$imagb.$real)}
function c_quot(a,b){var r,
abs_breal=_b_.abs(b.$real),abs_bimag=_b_.abs(b.$imag)
if($B.rich_comp('__ge__',abs_breal,abs_bimag)){
if(abs_breal==0.0){throw _b_.ZeroDivisionError.$factory()}else{var ratio=b.$imagb.$real,denom=b.$real+b.$imagratio
return make_complex((a.$real+a.$imagratio)denom,(a.$imag-a.$realratio)denom)}}else if(abs_bimag =abs_breal){
var ratio=b.$realb.$imag,denom=b.$realratio+b.$imag;
if(b.$imag==0.0){throw _b_.ZeroDivisionError.$factory()}
return make_complex(
(a.realratio+a.imag)denom,(a.imagratio-a.real)denom)}else{
...
if(_b_.isinstance(other,[_b_.int,_b_.float])){return make_complex(resMath.cos(angleother),resMath.sin(angleother))}else if(_b_.isinstance(other,complex)){
var x=other.$real,y=other.$imag
var pw=Math.pow(exp.norm,x)Math.pow(Math.E,-yangle),theta=yMath.log(exp.norm)-xangle
return make_complex(pwMath.cos(theta),pwMath.sin(theta))}else{throw _b_.TypeError.$factory(unsupported operand type(s) +
...
if(self.$imag instanceof Number && self.$imag==parseInt(self.$imag)){if(self.$imag==0 && 1self.$imag===-Infinity){imag=-0}}
if(self.$real==0){if(1self.$real  0){if(imag.startsWith('-')){return -0+imag+j}
return -0++imag+j}else{return imag+j}}
if(self.$imag  0 isNaN(self.$imag)){return (+real+++imag+j)}
if(self.$imag==0){if(1self.$imag  0){return (+real+-0j)}
return (+real++0j)}
...
if(_b_.isinstance(other,[_b_.int,_b_.float])){return make_complex(otherself.$real,otherself.$imag)}
return _b_.NotImplemented}
...
var _num=self.$realother.$real+self.$imagother.$imag,_div=other.$realother.$real+other.$imagother.$imag
var _num2=self.$imagother.$real-self.$realother.$imag
return make_complex(_num_div,_num2_div)}
...
__class__complex,$realreal,$imagimag}}
...
return true}
dict_view_op={__eq__function(t1,t2){return t1.length==t2.length && is_sublist(t1,t2)},__ne__function(t1,t2){return ! dict_view_op.__eq__(t1,t2)},__lt__function(t1,t2){return t1.length  t2.length && is_sublist(t1,t2)},__gt__function(t1,t2){return dict_view_op.__lt__(t2,t1)},__le__function(t1,t2){return t1.length =t2.length && is_sublist(t1,t2)},__ge__function(t1,t2){return dict_view_op.__le__(t2,t1)},__and__function(t1,t2){var items=[]
for(var i=0,ilen=t1.length;i  ilen;i++){var x=t1[i]
flag=false
for(var j=0,jlen=t2.length;j  jlen;j++){if($B.rich_comp(__eq__,x,t2[j])){t2.splice(j,1)
items.push(x)
break}}}
return items},__or__function(t1,t2){var items=t1
...
__class__klass,__dict__$B.empty_dict(),counter-1,dictd,itemsitems,lenitems.length,set_likeset_like}})
...
$B.set_func_names(klass,builtins)
return klass}
var dict={__class___b_.type,__mro__[_b_.object],$infos{__module__builtins,__name__dict},$is_classtrue,$nativetrue,$match_mapping_patterntrue }
...
return-1}
dict.__bool__=function(){var $=$B.args(__bool__,1,{selfnull},[self],arguments,{},null,null)
return dict.__len__($.self) 0}
dict.__class_getitem__=function(cls,item){
if(! Array.isArray(item)){item=[item]}
return $B.GenericAlias.$factory(cls,item)}
dict.__contains__=function(){var $=$B.args(__contains__,2,{selfnull,keynull},[self,key],arguments,{},null,null),self=$.self,key=$.key
...
dict.__delitem__=function(){var $=$B.args(__eq__,2,{selfnull,argnull},[self,arg],arguments,{},null,null),self=$.self,arg=$.arg
...
self.$version++
return $N}
dict.__eq__=function(){var $=$B.args(__eq__,2,{selfnull,othernull},[self,other],arguments,{},null,null),self=$.self,other=$.other
...
if(! flag){return false}}}
return true}
dict.__getitem__=function(){var $=$B.args(__getitem__,2,{selfnull,argnull},[self,arg],arguments,{},null,null),self=$.self,arg=$.arg
...
return $N}}
var $=$B.args(dict,1,{selfnull},[self],arguments,{},first,second)
var args=$.first
...
var instance={__class__cls,$numeric_dict {},$object_dict {},$string_dict {},$str_hash{},$version0,$order0}
...
return res}
dict.__setitem__=function(self,key,value){var $=$B.args(__setitem__,3,{selfnull,keynull,valuenull},[self,key,value],arguments,{},null,null)
...
dict.clear=function(){
var $=$B.args(clear,1,{selfnull},[self],arguments,{},null,null),self=$.self
self.$numeric_dict={}
self.$string_dict={}
self.$str_hash={}
self.$object_dict={}
...
var $=$B.args(copy,1,{selfnull},[self],arguments,{},null,null),self=$.self,res=$B.empty_dict()
$copy_dict(res,self)
return res}
dict.fromkeys=function(){var $=$B.args(fromkeys,3,{clsnull,keysnull,valuenull},[cls,keys,value],arguments,{value_b_.None},null,null),keys=$.keys,value=$.value
...
throw err}}}
dict.get=function(){var $=$B.args(get,3,{selfnull,keynull,_defaultnull},[self,key,_default],arguments,{_default$N},null,null)
try{
return dict.$getitem($.self,$.key,true)}catch(err){if(_b_.isinstance(err,_b_.KeyError)){return $._default}
else{throw err}}}
var dict_items=$B.make_view(dict_items,true)
dict_items.$iterator=$B.make_iterator_class(dict_itemiterator)
dict.items=function(self){var $=$B.args('items',1,{selfnull},['self'],arguments,{},null,null)
...
dict.keys=function(self){var $=$B.args('keys',1,{selfnull},['self'],arguments,{},null,null)
var it=dict_keys.$factory(self,to_list(self,0),true)
it.dict_version=self.$version
return it}
dict.pop=function(){var missing={},$=$B.args(pop,3,{selfnull,keynull,_defaultnull},[self,key,_default],arguments,{_defaultmissing},null,null),self=$.self,key=$.key,_default=$._default
...
dict.setdefault=function(){var $=$B.args(setdefault,3,{selfnull,keynull,_defaultnull},[self,key,_default],arguments,{_default$N},null,null),self=$.self,key=$.key,_default=$._default
try{
...
return _default}}
dict.update=function(self){var $=$B.args(update,1,{selfnull},[self],arguments,{},args,kw),self=$.self,args=$.args,kw=$.kw
...
dict.values=function(self){var $=$B.args('values',1,{selfnull},['self'],arguments,{},null,null)
...
__class__dict,$numeric_dict {},$object_dict {},$string_dict {},$str_hash{},$version0,$order0}}
dict.fromkeys=_b_.classmethod.$factory(dict.fromkeys)
$B.getset_descriptor=$B.make_class(getset_descriptor,function(klass,attr){return{
__class__$B.getset_descriptor,__doc___b_.None,clsklass,attrattr}}
)
...
__class__jsobj_as_pydict,objjsobj,excludeexclude  exclude function(){return false},new_keys[]}}
)
...
var list={__class___b_.type,__mro__[object],$infos{__module__builtins,__name__list},$is_classtrue,$nativetrue,$match_sequence_patterntrue,
__dir__object.__dir__}
...
if(! Array.isArray(item)){item=[item]}
return $B.GenericAlias.$factory(cls,item)}
list.__contains__=function(self,item){var $=$B.args(__contains__,2,{selfnull,itemnull},[self,item],arguments,{},null,null),self=$.self,item=$.item
...
return false}
list.__hash__=$N
list.__iadd__=function(){var $=$B.args(__iadd__,2,{selfnull,xnull},[self,x],arguments,{},null,null)
var x=list.$factory($B.$iter($.x))
for(var i=0;i  x.length;i++){$.self.push(x[i])}
return $.self}
list.__imul__=function(){var $=$B.args(__imul__,2,{selfnull,xnull},[self,x],arguments,{},null,null),x=$B.$GetInt($.x),len=$.self.length,pos=len
if(x==0){list.clear($.self);return $.self}
for(var i=1;i  x;i++){for(j=0;j  len;j++){$.self[pos++]=$.self[j]}}
return $.self}
list.__init__=function(self,arg){var $=$B.args('__init__',1,{selfnull},['self'],arguments,{},'args',null),self=$.self,args=$.args
...
_b_.dict.$setitem(self.__dict__,attr,value)
return $N}
list.__setitem__=function(){var $=$B.args(__setitem__,3,{selfnull,keynull,valuenull},[self,key,value],arguments,{},null,null),self=$.self,arg=$.key,value=$.value
...
self.push(x)
return $N}
list.clear=function(){var $=$B.args(clear,1,{selfnull},[self],arguments,{},null,null)
while($.self.length){$.self.pop()}
return $N}
list.copy=function(){var $=$B.args(copy,1,{selfnull},[self],arguments,{},null,null)
var res=$.self.slice()
res.__class__=self.__class__
res.__brython__=true
return res}
list.count=function(){var $=$B.args(count,2,{selfnull,xnull},[self,x],arguments,{},null,null)
var res=0,_eq=function(other){return $B.rich_comp(__eq__,$.x,other)},i=$.self.length
while(i--){if(_eq($.self[i])){res++}}
return res}
list.extend=function(){var $=$B.args(extend,2,{selfnull,tnull},[self,t],arguments,{},null,null)
var other=list.$factory($B.$iter($.t))
for(var i=0;i  other.length;i++){$.self.push(other[i])}
return $N}
list.index=function(){var missing={},$=$B.args(index,4,{selfnull,xnull,startnull,stopnull},[self,x,start ,stop],arguments,{start0,stopmissing},null,null),self=$.self,start=$.start,stop=$.stop
...
$B.class_name(self))}
list.insert=function(){var $=$B.args(insert,3,{selfnull,inull,itemnull},[self,i,item],arguments,{},null,null)
$.self.splice($.i,0,$.item)
return $N}
list.pop=function(){var missing={}
var $=$B.args(pop,2,{selfnull,posnull},[self,pos],arguments,{posmissing},null,null),self=$.self,pos=$.pos
...
self.splice(pos,1)
return res}
list.remove=function(){var $=$B.args(remove,2,{selfnull,xnull},[self,x],arguments,{},null,null)
...
list.reverse=function(self){var $=$B.args(reverse,1,{selfnull},[self],arguments,{},null,null),_len=$.self.length-1,i=parseInt($.self.length2)
...
return cl}
list.sort=function(self){var $=$B.args(sort,1,{selfnull},[self],arguments,{},null,kw)
...
var $=$B.args(list,1,{objnull},[obj],arguments,{},null,null),obj=$.obj
...
var tuple={__class___b_.type,__mro__[object],$infos{__module__builtins,__name__tuple},$is_classtrue,$nativetrue,$match_sequence_patterntrue,}
...
var _b_=$B.builtins
var $GeneratorReturn={}
$B.generator_return=function(value){return{__class__$GeneratorReturn,valuevalue}}
...
gen.$has_run=false
return{
__class__$B.generator,js_gengen}}
...
return{xmousePos.x-docPos.x,ymousePos.y-docPos.y};}
...
top+=parent_pos.top}
return{leftleft,toptop,widthwidth,heightheight}}
...
var re=new RegExp(^[object (HTMLCollectionNodeList)]$)
...
return true}
var $NodeTypes={1ELEMENT,2ATTRIBUTE,3TEXT,4CDATA_SECTION,5ENTITY_REFERENCE,6ENTITY,7PROCESSING_INSTRUCTION,8COMMENT,9DOCUMENT,10DOCUMENT_TYPE,11DOCUMENT_FRAGMENT,12NOTATION}
var Attributes=$B.make_class(Attributes,function(elt){return{__class__Attributes,eltelt}}
)
Attributes.__contains__=function(){var $=$B.args(__getitem__,2,{selfnull,keynull},[self,key],arguments,{},null,null)
...
return false}
Attributes.__delitem__=function(){var $=$B.args(__getitem__,2,{selfnull,keynull},[self,key],arguments,{},null,null)
...
return _b_.None}}
Attributes.__getitem__=function(){var $=$B.args(__getitem__,2,{selfnull,keynull},[self,key],arguments,{},null,null)
...
self.$items=items
return self}
Attributes.__next__=function(){var $=$B.args(__next__,1,{selfnull},[self],arguments,{},null,null)
if($.self.$counter  $.self.$items.length){var res=$.self.$items[$.self.$counter]
$.self.$counter++
return res}else{throw _b_.StopIteration.$factory()}}
Attributes.__setitem__=function(){var $=$B.args(__setitem__,3,{selfnull,keynull,valuenull},[self,key,value],arguments,{},null,null)
if($.self.elt instanceof SVGElement &&
...
return '{'+items.join(, )+'}'}
Attributes.get=function(){var $=$B.args(get,3,{selfnull,keynull,defltnull},[self,key,deflt],arguments,{deflt_b_.None},null,null)
...
Attributes.items=function(){var $=$B.args(values,1,{selfnull},[self],arguments,{},null,null),attrs=$.self.elt.attributes,values=[]
for(var i=0;i  attrs.length;i++){values.push([attrs[i].name,attrs[i].value])}
return _b_.list.__iter__(values)}
Attributes.values=function(){var $=$B.args(values,1,{selfnull},[self],arguments,{},null,null),attrs=$.self.elt.attributes,values=[]
...
func.$infos={__name__res.name,__qualname__res.name}
return func}
return $B.$JS2Py(res)}
throw $B.attr_error(attr,self)}
...
var OpenFile=$B.OpenFile={__class___b_.type,
__mro__[object],$infos{__module__pydom,__name__OpenFile},$is_classtrue}
OpenFile.$factory=function(file,mode,encoding){var res={__class__$OpenFileDict,filefile,readernew FileReader()}
...
var res={__class__Query,_keys [],_values {}}
...
func.$infos={__name__ attr,__qualname__attr}
func.$is_func=true
...
console.log(src.split(n)[parseInt(info[0])-1])}}else{console.log(module,frame[2])}}
...
DOMNode.abs_left={__get__function(self){return $getPosition(self).left},__set__function(){throw _b_.AttributeError.$factory('DOMNode' objectattribute +
'abs_left' is read-only)}}
DOMNode.abs_top={__get__function(self){return $getPosition(self).top},__set__function(){throw _b_.AttributeError.$factory('DOMNode' objectattribute +
'abs_top' is read-only)}}
DOMNode.attach=DOMNode.__le__ 
DOMNode.bind=function(self,event){
var $=$B.args(bind,4,{selfnull,eventnull,funcnull,optionsnull},[self,event,func,options],arguments,{func_b_.None,options_b_.None},null,null),self=$.self,event=$.event,func=$.func,options=$.options
if(func===_b_.None){
...
var $=$B.args(clear,1,{selfnull},[self],arguments,{},null,null)
if(self.nodeType==9){self=self.body}
...
var $=$B.args(closest,2,{selfnull,selectornull},[self,selector],arguments,{},null,null)
...
DOMNode.scrolled_left={__get__function(self){return $getPosition(self).left-
document.scrollingElement.scrollLeft},__set__function(){throw _b_.AttributeError.$factory('DOMNode' objectattribute +
'scrolled_left' is read-only)}}
DOMNode.scrolled_top={__get__function(self){return $getPosition(self).top-
document.scrollingElement.scrollTop},__set__function(){throw _b_.AttributeError.$factory('DOMNode' objectattribute +
...
__class__TagSum,children[],toStringfunction(){return (TagSum)}}}
)
...
if(self.children[i].toString()==[object Text]){res+= [+self.children[i].textContent+]n}}
return res}
...
win.get_postMessage=function(msg,targetOrigin){if(_b_.isinstance(msg,dict)){var temp={__class__dict},items=_b_.list.$factory(_b_.dict.items(msg))
...
var _window=self;
var modules={}
var browser={$packagetrue,$is_packagetrue,__initialized__true,__package__'browser',__file__$B.brython_path.replace($g,'')+
'Libbrowser__init__.py',bindfunction(){
var $=$B.args(bind,3,{eltnull,evtnull,optionsnull},[elt,evt,options],arguments,{options_b_.None},null,null)
var options=$.options
...
throw err}
return callback}},consoleself.console && $B.JSObj.$factory(self.console),self$B.win,win$B.win,window$B.win,}
...
update(browser,{alertfunction(message){window.alert($B.builtins.str.$factory(message ))},confirm$B.JSObj.$factory(window.confirm),document$B.DOMNode.$factory(document),doc$B.DOMNode.$factory(document),
DOMEvent$B.DOMEvent,DOMNode$B.DOMNode,loadfunction(script_url){
var file_obj=$B.builtins.open(script_url)
var content=$B.$getattr(file_obj,'read')()
eval(content)},mouseCoordsfunction(ev){return $B.JSObj.$factory($mouseCoords(ev))},promptfunction(message,default_value){return $B.JSObj.$factory(window.prompt(message,default_value''))},reloadfunction(){
var scripts=document.getElementsByTagName('script'),js_scripts=[]
scripts.forEach(function(script){if(script.type===undefined 
script.type=='textjavascript'){js_scripts.push(script)
if(script.src){console.log(script.src)}}})
console.log(js_scripts)
for(var mod in $B.imported){if($B.imported[mod].$last_modified){console.log('check',mod,$B.imported[mod].__file__,$B.imported[mod].$last_modified)}else{console.log('no date for mod',mod)}}},run_scriptfunction(){var $=$B.args(run_script,2,{srcnull,namenull},[src,name],arguments,{namescript_+$B.UUID()},null,null)
$B.run_script($.src,$.name,$B.script_path,true)},URLParameterfunction(name){name=name.replace([[],[).replace([]],]);
...
var dict={__class___b_.type,$infos{__name__tagName,__module__browser.html,__qualname__tagName}}
dict.__init__=function(){var $ns=$B.args('pow',1,{selfnull},['self'],arguments,{},'args','kw'),self=$ns['self'],args=$ns['args']
...
document.createElementNS(httpwww.w3.org2000svg,svg),true)}else{var res=document.createElement(klass.$infos.__name__)}
...
html.attribute_mapper=function(attr){return attr.replace(_g,'-')}
...
return res}
return{
__class__super_class,__init__factory,__self_class__b_self}}
)
...
modules['javascript']={thisfunction(){
if($B.js_this===undefined){return $B.builtins.None}
return $B.JSObj.$factory($B.js_this)},Dateself.Date && $B.JSObj.$factory(self.Date),extendsfunction(js_constr){return function(obj){if(obj.$is_class){var factory=function(){var init=$B.$getattr(obj,__init__,_b_.None)
...
return factory}}},JSObject$B.JSObj,JSON{__class__$B.make_class(JSON),parsefunction(){return $B.structuredclone2pyobj(
JSON.parse.apply(this,arguments))},stringifyfunction(obj,replacer,space){return JSON.stringify($B.pyobj2structuredclone(obj,false),$B.JSObj.$factory(replacer),space)}},jsobj2pyobjfunction(obj){return $B.jsobj2pyobj(obj)},loadfunction(script_url){console.log('javascript.load is deprecrated. '+
'Use browser.load instead.')
var file_obj=$B.builtins.open(script_url)
var content=$B.$getattr(file_obj,'read')()
eval(content)},Mathself.Math && $B.JSObj.$factory(self.Math),NULLnull,Numberself.Number && $B.JSObj.$factory(self.Number),py2jsfunction(src,module_name){if(module_name===undefined){module_name='__main__'+$B.UUID()}
var js=$B.py2js(src,module_name,module_name,$B.builtins_scope).to_js()
return $B.format_indent(js,0)},pyobj2jsobjfunction(obj){return $B.pyobj2jsobj(obj)},RegExpself.RegExp && $B.JSObj.$factory(self.RegExp),Stringself.String && $B.JSObj.$factory(self.String),supersuper_class,UNDEFINED$B.Undefined,UndefinedType$B.UndefinedClass}
...
var _b_=$B.builtins
modules['_sys']={
Getframe function(){var $=$B.args(_getframe,1,{depthnull},['depth'],arguments,{depth0},null,null),depth=$.depth
return $B._frame.$factory($B.frames_stack,$B.frames_stack.length-depth-1)},breakpointhookfunction(){var hookname=$B.$options.breakpoint,modname,dot,funcname,hook
...
return _b_.None}
return $B.$call(hook).apply(null,arguments)},exc_infofunction(){for(var i=$B.frames_stack.length-1;i =0;i--){var frame=$B.frames_stack[i],exc=frame[1].$current_exception
if(exc){return _b_.tuple.$factory([exc.__class__,exc,$B.$getattr(exc,__traceback__)])}}
return _b_.tuple.$factory([_b_.None,_b_.None,_b_.None])},excepthookfunction(exc_class,exc_value,traceback){$B.handle_error(exc_value)},gettracefunction(){return $B.tracefunc _b_.None},max_string_length$B.max_string_length,
modules_b_.property.$factory(
function(){return $B.obj_dict($B.imported)},function(self,value){throw _b_.TypeError.$factory(Read only property 'sys.modules')}
),path_b_.property.$factory(
function(){return $B.path},function(self,value){$B.path=value;}
),meta_path_b_.property.$factory(
function(){return $B.meta_path},function(self,value){$B.meta_path=value}
),path_hooks_b_.property.$factory(
function(){return $B.path_hooks},function(self,value){$B.path_hooks=value}
),path_importer_cache_b_.property.$factory(
...
var WarningMessage=$B.make_class(WarningMessage,function(){var $=$B.make_args(WarningMessage,8,{messagenull,categorynull,filenamenull,linenonull,filenull,linenull,sourcenull},['message','category','filename','lineno','file','line','source'],arguments,{file_b_.None,line_b_.None,source_b_.None},null,null)
return{
__class__WarningMessage,message$.message,category$.category,filename$.filename,lineno$.lineno,file$.file,line$.line,source$.source,_category_name_b_.bool.$factory($.category)
$B.$getattr($.category,__name__)_b_.None}}
)
modules._warnings={_defaultactiondefault,_filters_mutatedfunction(){},_onceregistry$B.empty_dict(),filters[$B.fast_tuple(['default',_b_.None,_b_.DeprecationWarning,'__main__',0]),$B.fast_tuple(['ignore',_b_.None,_b_.DeprecationWarning,_b_.None,0]),$B.fast_tuple(['ignore',_b_.None,_b_.PendingDeprecationWarning,_b_.None,0]),$B.fast_tuple(['ignore',_b_.None,_b_.ImportWarning,_b_.None,0]),$B.fast_tuple(['ignore',_b_.None,_b_.ResourceWarning,_b_.None,0])
],warnfunction(message){
var filters
...
throw syntax_error}
var frame=$B.imported._sys.Getframe(),category=message.__class__ $B.get_class(message),warning_message={__class__WarningMessage,messagemessage,category,filenamemessage.filename frame.f_code.co_filename,linenomessage.lineno frame.f_lineno,file_b_.None,line_b_.None,source_b_.None,_category_namecategory.__name__}
...
$B.$getattr($B.stderr,'write')(trace+'n')
var flush=$B.$getattr($B.stderr,'flush',_b_.None)
if(flush !==_b_.None){flush()}}},warn_explicitfunction(){
console.log(warn_explicit,arguments)}}
function load(name,module_obj){
module_obj.__class__=$B.module
module_obj.__name__=name
$B.imported[name]=module_obj
for(var attr in module_obj){if(typeof module_obj[attr]=='function'){module_obj[attr].$infos={__module__name,__name__attr,__qualname__name+'.'+attr}}}}
...
__class__$B.cell,$cell_contentsvalue}}
)
...
$B.AST={__class___b_.type,__getattr__function(self,attr){var res=self.js_node[attr]
if(res===undefined){throw $B.attr_error(attr,self)}
return $B.AST.$convert(res)},__mro__[_b_.object],$infos{__qualname__'AST',__name__'AST'},$is_classtrue,$convertfunction(js_node){if(js_node===undefined){return _b_.None}
...
return{
__class__coroutine,$argsargs,$funcfunc,$stackstack}}
f.$infos=func.$infos
return f}
...
if(exc.filename !='string'){var src=$B.file_cache[exc.filename],lines=src.split('n'),line=lines[ast_obj.lineno-1]
...
var s=scope
while(true){if(s.locals.has(name)){return{foundtrue,scopes}}
if(! s.parent){return{foundfalse}}
s=s.parent}}
function name_scope(name,scopes){
var flags,block
if(scopes.length==0){
return{foundfalse,resolve'all'}}
...
return{foundfalse,resolve'global'}}else if(block.type==TYPE_MODULE){return{foundfalse,resolve'global'}}
return{foundfalse,resolve'local'}}else{return{foundl_scope.scope}}}else if(scope.globals.has(name)){var global_scope=scopes[0]
if(global_scope.locals.has(name)){return{foundglobal_scope}}
return{foundfalse,resolve'global'}}else if(scope.nonlocals.has(name)){
for(var i=scopes.length-2;i =0;i--){block=scopes.symtable.table.blocks.get(_b_.id(scopes[i].ast))
if(block && block.symbols.$string_dict[name]){return{foundscopes[i]}}}}
if(scope.has_import_star){return{foundfalse,resolveis_local  'all' 'global'}}
...
if(scopes[i].globals.has(name)){return{foundfalse,resolve'global'}}
if(scopes[i].locals.has(name)&& scopes[i].type !='class'){return{foundscopes[i]}}else if(block && block.symbols.$string_dict[name]){flags=block.symbols.$string_dict[name][0]
var __scope=(flags  SCOPE_OFF)& SCOPE_MASK
if([LOCAL,CELL].indexOf(__scope)-1){
return{foundfalse,resolve'all'}}}
if(scopes[i].has_import_star){return{foundfalse,resolve'all'}}}
if(builtins_scope.locals.has(name)){return{foundbuiltins_scope}}
var scope_names=make_search_namespaces(scopes)
return{foundfalse,resolvescope_names}}
function resolve_in_namespace(name,ns){if(! ns.hasOwnProperty){if(ns[name]!==undefined){return{foundtrue,valuens[name]}}}else if(ns.hasOwnProperty(name)){return{foundtrue,valuens[name]}}else if(ns.$dict){try{return{foundtrue,valuens.$getitem(ns.$dict,name)}}catch(err){if(ns.$missing){try{return{
foundtrue,value$B.$call(ns.$missing)(ns.$dict,name)}}catch(err){if(! $B.$is_exc(err,[_b_.KeyError])){throw err}}}}}
return{foundfalse}}
...
var p={$parentnode,'type''except_handler'}
...
if(js.length  0){res+=js+'n'}}
return res.trimRight()}
function init_comprehension(comp){
var comp_id=comp.type+'_'+comp.id,varnames=Object.keys(comp.varnames {}).map(x= `'${x}'`).join(', ')
return `var ${comp.locals_name} = {},n`+
`locals = ${comp.locals_name}n`+
`locals.$lineno = ${comp.ast.lineno}n`+
`locals.$comp_code = {n`+
`co_argcount 1,n`+
`co_firstlineno${comp.ast.lineno},n`+
`co_name ${comp.type},n`+
`co_flags ${comp.type == 'genexpr'  115  83},n`+
`co_freevars $B.fast_tuple([]),n`+
`co_kwonlyargcount 0,n`+
`co_posonlyargount 0,n`+
`co_varnames $B.fast_tuple(['.0', ${varnames}])n`+
`}n`+
`locals['.0'] = exprn`+
`var top_frame = [${comp.type.toLowerCase()}, ${comp.locals_name}, `+
`${comp.module_name}, ${comp.globals_name}]n`+
`locals.$f_trace = $B.enter_frame(top_frame)n`}
...
var comp={astthis,id,type,varnames,module_namescopes[0].name,locals_namemake_scope_name(scopes),globals_namemake_scope_name(scopes,scopes[0])}
var js=init_comprehension(comp)
if(this instanceof $B.ast.ListComp){js+=`var result_${id} = []n`}else if(this instanceof $B.ast.SetComp){js+=`var result_${id} = _b_.set.$factory()n`}else if(this instanceof $B.ast.DictComp){js+=`var result_${id} = $B.empty_dict()n`}
var first=this.generators[0]
js+=`var next_func_${id} = $B.next_of(expr)n`+
`while(true){ntry{nvar next_${id} = next_func_${id}()n`+
`}catch(err){nif($B.is_exc(err, [_b_.StopIteration])){n`+
`breakn}else{n$B.leave_frame({locals, value _b_.None})n `+
`throw errn}n}n`
...
js+=assign.to_js(scopes)+'n'
for(var _if of first.ifs){nb_paren++
js+=`if($B.$bool(${$B.js_from_ast(_if, scopes)})){n`}
...
js=`(${has_await  'async '  ''}function(expr){n`+js
js+=has_await  'var save_stack = $B.save_stack();n' ''
js+=`try{n`
if(this instanceof $B.ast.ListComp){js+=`result_${id}.push(${elt})n`}else if(this instanceof $B.ast.SetComp){js+=`_b_.set.add(result_${id}, ${elt})n`}else if(this instanceof $B.ast.DictComp){js+=`_b_.dict.$setitem(result_${id}, ${key}, ${value})n`}
js+=`}catch(err){n`+
(has_await  '$B.restore_stack(save_stack, locals)n' '')+
`$B.leave_frame(locals)nthrow errn}`+
(has_await  'n$B.restore_stack(save_stack, locals);' '')
for(var i=0;i  nb_paren;i++){js+='}n'}
js+=`n$B.leave_frame({locals, value _b_.None})`
js+=`nreturn result_${id}`
js+=`n}n)(${outmost_expr})n`
scopes.pop()
return js}
var exec_num={value0}
...
return `if($B.set_lineno(locals, ${this.lineno}) && !$B.$bool(${test})){n`+
`throw _b_.AssertionError.$factory(${msg})}n`}
$B.ast.AnnAssign.prototype.to_js=function(scopes){var scope=last_scope(scopes)
var js=''
if(! scope.has_annotation){js+='locals.__annotations__ = $B.empty_dict()n'}
scope.has_annotation=true
if(this.value){js+=`var ann = ${$B.js_from_ast(this.value, scopes)}n`
if(this.target instanceof $B.ast.Name){var scope=bind(this.target.id,scopes)
js+=`$B.$setitem(locals.__annotations__, `+
`'${this.target.id}', ${$B.js_from_ast(this.annotation, scopes)})n`
...
js+=`$B.$setitem(locals.__annotations__, `+
`'${this.target.id}', ${ann})`}
return `$B.set_lineno(locals, ${this.lineno})n`+js}
$B.ast.Assign.prototype.to_js=function(scopes){compiler_check(this)
var js=this.lineno  `$B.set_lineno(locals, ${this.lineno})n` '',value=$B.js_from_ast(this.value,scopes)
...
var id='v'+$B.UUID()
js+=`var ${id} = ${value}n`
for(var target of this.targets){js+=assign_one(target,id)+'n'}
return js}
...
var s=`var mgr_${id} = `+
$B.js_from_ast(item.context_expr,scopes)+',n'+
`mgr_type_${id} = _b_.type.$factory(mgr_${id}),n`+
`aexit_${id} = $B.$getattr(mgr_type_${id}, '__aexit__'),n`+
`aenter_${id} = $B.$getattr(mgr_type_${id}, '__aenter__'),n`+
`value_${id} = await $B.promise($B.$call(aenter_${id})(mgr_${id})),n`+
`exc_${id} = truen`
if(has_generator){
s+=`locals.$context_managers = locals.$context_managers  []n`+
`locals.$context_managers.push(mgr_${id})n`}
s+='try{ntry{n'
if(item.optional_vars){
var value={to_jsfunction(){return `value_${id}`}}
copy_position(value,_with)
var assign=new $B.ast.Assign([item.optional_vars],value)
copy_position(assign,_with)
s+=assign.to_js(scopes)+'n'}
s+=js
s+=`}catch(err_${id}){n`+
`locals.$lineno = ${lineno}n`+
`exc_${id} = falsen`+
`err_${id} = $B.exception(err_${id}, true)n`+
`var $b = await $B.promise(aexit_${id}(mgr_${id}, err_${id}.__class__, `+
`err_${id}, $B.$getattr(err_${id}, '__traceback__')))n`+
`if(! $B.$bool($b)){nthrow err_${id}n}n}n`
s+=`}nfinally{n`+
`locals.$lineno = ${lineno}n`+
`if(exc_${id}){n`+
`await $B.promise(aexit_${id}(mgr_${id}, _b_.None, _b_.None, _b_.None))n}n}n`
return s}
...
js=add_body(this.body,scopes)+'n'
...
`$B.is_safe_int(locals.$result = ${ref} ${op} ${value}) n`+
...
var js=''
for(var scope of scopes.slice().reverse()){if(scope.ast instanceof $B.ast.For){js+=`no_break_${scope.id} = falsen`
break}}
js+=`break`
return js}
...
if(named_kwargs.length+starred_kwargs.length==0){return{has_starred,jsjs+`${args}`}}else{var kw=`{${named_kwargs.join(', ')}}`
for(var starred_kwarg of starred_kwargs){kw+=`, ${starred_kwarg}`}
kw=`{$nat 'kw', kw[${kw}]}`
if(args.length  0){if(has_starred){kw=`.concat([${kw}])`}else{kw=', '+kw}}
return{has_starred,jsjs+`${args}${kw}`}}}
...
var js=`$B.set_lineno(locals, ${this.lineno})n`,locals_name=make_scope_name(scopes,class_scope),ref=this.name+$B.UUID(),glob=scopes[0].name,globals_name=make_scope_name(scopes,scopes[0]),decorators=[],decorated=false
for(var dec of this.decorator_list){decorated=true
var dec_id='decorator'+$B.UUID()
decorators.push(dec_id)
js+=`var ${dec_id} = ${$B.js_from_ast(dec, scopes)}n`}
...
js+=`var ${ref} = (function(){n`+
`var ${locals_name} = {__annotations__ $B.empty_dict()},n`+
`locals = ${locals_name}n`+
`locals.$name = ${this.name}n`+
`locals.$qualname = ${qualname}n`+
`locals.$is_class = truen`+
`var top_frame = [${ref}, locals, ${glob}, ${globals_name}]n`+
`top_frame.__file__ = '${scopes.filename}'n`+
`locals.$lineno = ${this.lineno}n`+
`locals.$f_trace = $B.enter_frame(top_frame)n`+
`if(locals.$f_trace !== _b_.None){$B.trace_line()}n`
js+=add_body(this.body,scopes)
scopes.pop()
js+='nif(locals.$f_trace !== _b_.None){n'+
'$B.trace_return(_b_.None)n'+
'}n'+
'$B.leave_frame({locals})n'+
'return localsn})()n'
...
`$B.fast_tuple([${bases}]), [], [${keywords.join(', ')}])n`+
`${class_ref}.__doc__ = ${docstring}n`
...
var js=`var next_func_${id} = $B.next_of(${iter})n`+
`while(true){ntry{nvar next_${id} = next_func_${id}()n`+
`}catch(err){nif($B.is_exc(err, [_b_.StopIteration])){n`+
`breakn}else{n$B.leave_frame({locals, value _b_.None})n `+
`throw errn}n}n`
...
alert()}
js+=assign.to_js(scopes)+'  assign to targetn'
for(var _if of this.ifs){js+=`if($B.$bool(${$B.js_from_ast(_if, scopes)})){n`}
return js}
...
js+=`$B.$delete(${target.id})n`}else if(target instanceof $B.ast.Subscript){js+=`$B.$delitem(${$B.js_from_ast(target.value, scopes)}, `+
`${$B.js_from_ast(target.slice, scopes)})n`}else if(target instanceof $B.ast.Attribute){js+=`_b_.delattr(${$B.js_from_ast(target.value, scopes)}, `+
`'${target.attr}')n`}}
return `$B.set_lineno(locals, ${this.lineno})n`+js}
...
$B.ast.Expr.prototype.to_js=function(scopes){return `$B.set_lineno(locals, ${this.lineno});n`+
...
if(this instanceof $B.ast.AsyncFor){js=`var iter_${id} = ${iter},n`+
`type_${id} = _b_.type.$factory(iter_${id})n`+
`iter_${id} = $B.$call($B.$getattr(type_${id}, __aiter__))(iter_${id})n`+
`var next_func_${id} = $B.$call(`+
`$B.$getattr(type_${id}, '__anext__'))n`+
`while(true){n`+
`  try{n`+
`    var next_${id} = await $B.promise(next_func_${id}(iter_${id}))n`+
`  }catch(err){n`+
`    if($B.is_exc(err, [_b_.StopAsyncIteration])){nbreak}n`+
`    else{nthrow err}n`+
`  }n`}else{js=`var no_break_${id} = truen`+
`var next_func_${id} = $B.next_of(${iter})n`+
`while(true){n`+
`try{n`+
`$B.set_lineno(locals, ${this.lineno})n`+
`var next_${id} = next_func_${id}()n`+
`}catch(err){n`+
`if($B.is_exc(err, [_b_.StopIteration])){n`+
`breakn`+
`}else{n `+
`throw errn`+
`}n`+
...
js+='n}' 
scopes.pop()
if(this.orelse.length  0){js+=`nif(no_break_${id}){n`+
add_body(this.orelse,scopes)+'}n'}
return js}
...
var dec_id='decorator'+$B.UUID()
decorators.push(dec_id)
decs+=`var ${dec_id} = ${$B.js_from_ast(dec, scopes)}  decoratorn`}
var docstring='_b_.None'
if(this.body[0]instanceof $B.ast.Expr &&
this.body[0].value instanceof $B.ast.Constant &&
...
var js=decs
js+=`var ${name1} = function(defaults){n`
...
js+=`${locals_name} = locals = $B.args(${parse_args.join(', ')})n`
js+=`var $top_frame = [${this.name}, locals, ${gname}, ${globals_name}, ${name2}]
    locals.$lineno = ${this.lineno}
    locals.$f_trace = $B.enter_frame($top_frame)
    var stack_length = $B.frames_stack.lengthn`
if(last_scope(scopes).has_annotation){js+=`locals.__annotations__ = $B.empty_dict()n`}
if(is_generator){js+=`locals.$is_generator = truen`
if(is_async){js+=`var gen_${id} = $B.async_generator.$factory(async function(){n`}else{js+=`var gen_${id} = $B.generator.$factory(function(){n`}}
js+=`try{n$B.js_this = thisn`
if(in_class){
...
js+=`locals.__class__ = `+
`$B.get_method_class(${scope_ref}, ${class_ref})n`}
js+=function_body+'n'
if((! this.$is_lambda)&& !($B.last(this.body)instanceof $B.ast.Return)){
js+='var result = _b_.Nonen'+
'if(locals.$f_trace !== _b_.None){n'+
'$B.trace_return(_b_.None)n}n'+
'$B.leave_frame(locals);return resultn'}
js+=`}catch(err){
    $B.set_exc(err)
...
if(is_generator){js+=`, '${this.name}')n`+
`var _gen_${id} = gen_${id}()n`+
`_gen_${id}.$frame = $top_framen`+
`$B.leave_frame()n`+
`return _gen_${id}}n` }
scopes.pop()
...
js+=`${name2}.$is_func = truen`
if(is_async){js+=`${name2}.$is_async = truen`}
js+=`${name2}.$infos = {n`+
`__name__ ${this.name}, __qualname__ ${qualname},n`+
`__defaults__ $B.fast_tuple([${default_names}]), `+
`__kwdefaults__ $B.fast_tuple([${kw_default_names}]),n`+
`__doc__ ${docstring},n`+
`__code__{n`+
`co_argcount ${positional.length},n `+
`co_filename ${make_local(scopes[0].name)}.__file__,n`+
`co_firstlineno ${this.lineno},n`+
`co_flags ${flags},n`+
`co_freevars $B.fast_tuple([${free_vars}]),n`+
`co_kwonlyargcount ${this.args.kwonlyargs.length},n`+
`co_name '${this.name}',n`+
`co_nlocals ${varnames.length},n`+
`co_posonlyargcount ${this.args.posonlyargs.length},n`+
`co_varnames $B.fast_tuple([${varnames}])n`+
`}n}n`
...
js+='var '}
js+=`${func_ref} = ${name1}(${default_str})n`+
`${func_ref}.$set_defaults = function(value){n`+
`return ${func_ref} = ${name1}(value)n}n`
...
scopes.push(comp_scope)
var comp={astthis,id,type'genexpr',varnames,module_namescopes[0].name,locals_namemake_scope_name(scopes),globals_namemake_scope_name(scopes,scopes[0])}
var js=init_comprehension(comp)
var first=this.generators[0]
js+=`var next_func_${id} = $B.next_of(expr)n`+
`while(true){ntry{nvar next_${id} = next_func_${id}()n`+
...
js=`$B.generator.$factory(${has_await  'async '  ''}function(expr){n`+js
js+=has_await  'var save_stack = $B.save_stack();n' ''
js+=`try{n`+
` yield ${elt}n`+
`}catch(err){n`+
(has_await  '$B.restore_stack(save_stack, locals)n' '')+
`$B.leave_frame(locals)nthrow errn}n`+
(has_await  'n$B.restore_stack(save_stack, locals);' '')
for(var i=0;i  nb_paren;i++){js+='}n'}
js+=`n$B.leave_frame({locals, value _b_.None})`+
`}, genexpr)(${outmost_expr})n`
scopes.pop()
return js}
...
`$B.$bool(${$B.js_from_ast(this.test, scopes)})){n`
scopes.push(new_scope)
js+=add_body(this.body,scopes)+'n}'
scopes.pop()
...
add_body(this.orelse.slice(1),scopes)}else{js+='nelse{n'+add_body(this.orelse,scopes)+'n}'}}
return js}
...
$B.ast.Import.prototype.to_js=function(scopes){var js=`$B.set_lineno(locals, ${this.lineno})n`
...
var js=`$B.set_lineno(locals, ${this.lineno})n`+
...
js+=`n$B.import_all(locals, module)`}else{bind(alias.name,scopes)}}
return js}
...
var js=`var subject = ${$B.js_from_ast(this.subject, scopes)}n`
first=true
...
var js=` Javascript code generated from astn`+
`var $B = __BRYTHON__,n_b_ = $B.builtins,n`
if(! namespaces){js+=`${global_name} = {},nlocals = ${global_name},n`+
`$top_frame = [${module_id}, locals, ${module_id}, locals]`}else{js+=`locals = ${namespaces.local_name},n`+
`globals = ${namespaces.global_name},n`+
`$top_frame = [${module_id}, locals, ${module_id}_globals, globals]`}
js+=`nlocals.__file__ = '${scopes.filename  string}'n`+
`locals.__name__ = '${name}'n`
if(! namespaces){
js+=`locals.$f_trace = $B.enter_frame($top_frame)n`}
js+=`$B.set_lineno(locals, ${this.lineno})n`+
`var stack_length = $B.frames_stack.lengthn`+
`try{n`+
add_body(this.body,scopes)+'n'+
(namespaces  '' `$B.leave_frame({locals, value _b_.None})n`)+
`}catch(err){n`+
`$B.set_exc(err)n`+
`if((! err.$in_trace_func) && locals.$f_trace !== _b_.None){n`+
`locals.$f_trace = $B.trace_exception()n`+
`}n`+
(namespaces  '' `$B.leave_frame({locals, value _b_.None})n`)+
'throw errn'+
`}`
...
return ''}
$B.ast.Pass.prototype.to_js=function(scopes){return `$B.set_lineno(locals, ${this.lineno})n`+
'void(0)'}
$B.ast.Raise.prototype.to_js=function(scopes){var js=`$B.set_lineno(locals, ${this.lineno})n`+
'$B.$raise('
...
var js=`$B.set_lineno(locals, ${this.lineno})ntry{n`
js+=`var stack_length_${id} = $B.frames_stack.lengthn`
if(has_finally){js+=`var save_stack_${id} = $B.frames_stack.slice()n`}
if(has_else){js+=`var failed${id} = falsen`}
...
`if(locals.$f_trace !== _b_.None){n`+
`locals.$f_trace = $B.trace_exception()}n`
if(has_else){js+=`failed${id} = truen`}
...
js+=`)){n`}else{has_untyped_except=true
js+='){n'}
if(handler.name){bind(handler.name,scopes)
var mangled=mangle(scopes,try_scope,handler.name)
js+=`locals.${mangled} = ${err}n`}
js+=add_body(handler.body,scopes)+'n'
if(!($B.last(handler.body)instanceof $B.ast.Return)){
js+='$B.del_exc()n'}}
if(! has_untyped_except){
js+=`}else{nthrow ${err}n`}
js+='}n'}
if(has_else has_finally){js+='}n' 
js+='finally{n'
var finalbody=`var exit = falsen`+
`if($B.frames_stack.length  stack_length_${id}){n`+
`exit = truen`+
`$B.frames_stack.push($top_frame)n`+
`}n`+
add_body(this.finalbody,scopes)
if(this.finalbody.length  0 &&
!($B.last(this.finalbody)instanceof $B.ast.Return)){finalbody+=`nif(exit){n`+
`$B.leave_frame(locals)n`+
`}`}
...
'n}' 
if(has_else && has_finally){js+=`try{n`+
elsebody+
'n}n'+
`finally{n`+finalbody+'}n'}else if(has_else && ! has_finally){js+=elsebody}else{js+=finalbody}
js+='n}n' }else{js+='}n' }
scopes.pop()
return js}
...
`locals.$context_managers.push(mgr_${id})n`}
s+='try{ntry{n'
if(item.optional_vars){var value={to_jsfunction(){return `value_${id}`}}
copy_position(value,_with)
var assign=new $B.ast.Assign([item.optional_vars],value)
copy_position(assign,_with)
s+=assign.to_js(scopes)+'n'}
...
`locals.$lineno = ${lineno}n`+
`if(exc_${id}){n`+
`exit_${id}(mgr_${id}, _b_.None, _b_.None, _b_.None)n}n}n`
return s}
var _with=this,scope=last_scope(scopes),lineno=this.lineno
delete scope.is_generator
js=add_body(this.body,scopes)+'n'
...
return{js,importsscopes.imports}}
...
ste={tablest,id_b_.id(key),
namename,directivesNULL,typeblock,nested0,free0,varargs0,varkeywords0,opt_lineno0,opt_col_offset0,linenolineno,col_offsetcol_offset,end_linenoend_lineno,end_col_offsetend_col_offset}
if(st.cur !=NULL &&
(st.cur.nested 
...
if(src !==undefined){var lines=src.split('n')
